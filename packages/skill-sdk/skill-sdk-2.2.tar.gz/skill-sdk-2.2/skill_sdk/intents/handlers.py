...
from skill_sdk.intents import entities, AttributeV2
from skill_sdk.intents import Request, Session, RequestContextVar
...
def _converters(annotation) -> Tuple:
    ...
    elif lenient_issubclass(annotation, (Request, Session)):
...
def _parse_request(
    ...
    result = {
        ...
        # look up attributes, if annotated as AttributeV2
        else request.attributes.get(name)
        if _is_attribute_v2(param.annotation)
        # look up the object attributes and attributes list
        else _search_for_attribute(request, name)
        for name, param in parameters
    }
    return result


def _search_for_attribute(request: Request, name: Text) -> Any:
    object_attribute = getattr(request, name, None)
    if object_attribute is not None:
        return object_attribute

    try:
        attribute_list: Optional[List[AttributeV2]] = request.attributes.get(name)
        if attribute_list is not None:
            return [item.value for item in attribute_list]
        return None
    except AttributeError:
        return None
