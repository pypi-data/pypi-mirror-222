...
async def invoke_intent(
    ...
    try:
        handler = rq.app.get_handler(request.intent)
    except KeyError:
        logger.error("Intent not found: %s", repr(request.intent))
        return JSONResponse({"code": 1, "text": "Intent not found!"}, status_code=404)

    response = await invoke(
        handler,
        request.with_translation(_get_translation(rq.app, request.locale)),
    )
