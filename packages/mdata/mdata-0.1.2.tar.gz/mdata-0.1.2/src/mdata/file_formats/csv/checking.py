...
def check_if_readable_header_definition_file(file: DataSource, do_full_check=True):
    ...
    try:
        ...
        raise MalformedHeaderFileException('Unparsable csv:\n' + str(e))
    if do_full_check:
        for k, row in enumerate(lines_from, start=1):
            ...
            non_empty_idx = [0]
            if row_identifier in mdd.ObservationTypes:
                label = row[1]
                ...
                if row[2] in mdd.ObservationTypes:
                    e = tuple(row[1:5])
...
def check_if_readable_header_definition_file_yaml(file: DataSource, do_full_check=True):
    try:
        ...
        raise MalformedHeaderFileException('Unparsable yaml:\n' + str(e))
