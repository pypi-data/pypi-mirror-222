...
test_tags = cObject.get('Tags', 'run_type')
...
def getall_actions(testcase, lpage, gAitem, gTestitem, gFelements, linex, gRstring, aresults, exresults, case, gItems,
    ...
    try:
        ...
        global gTx
        if gAitem == 'url':
            gTx = 'Navigated to url '+cObject.get('Url', 'base_url')
        elif gAitem == 'xurl':
    ...
    with open(pathdata_file, 'w') as f:
        ...
        for getxl in getallxm:
            if pfix in getxl:
                ...
                f.write('\t\t""""""\n')
                f.write('\t\tif get_tags(rtype):\n')
                f.write('\t\t\tlog.info("Test_Tags: " + str(get_tags(rtype)))\n')
