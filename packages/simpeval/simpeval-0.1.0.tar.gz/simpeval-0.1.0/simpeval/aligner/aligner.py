from simpeval.aligner.word_sim import *
from simpeval.aligner.utils import *
from simpeval.aligner.corenlp_utils import *
    ...
    for n in range(numberOfNounsInSource):
        ...
        for i in sourceWordIndices:
            ...
            for j in targetWordIndices:
                ...
                if (i, j) in evidenceCountsMatrix and theta1 * wordSimilarities[
                    (i, j)
                ] + (1 - theta1) * evidenceCountsMatrix[
                    (i, j)
        ...
        for jtem in targetNamedEntities:
            if item[2] in jtem[2]:
                ...
                for k in range(len(sourceNamedEntities)):
                    if (
                        sourceNamedEntities[k][1][len(sourceNamedEntities[k][1]) - 1]
                        == newItem[1][0] - 1
                    ):
        ...
        for jtem in sourceNamedEntities:
            if item[2] in jtem[2]:
                ...
                for k in range(len(targetNamedEntities)):
                    if (
                        targetNamedEntities[k][1][len(targetNamedEntities[k][1]) - 1]
                        == newItem[1][0] - 1
                    ):
                    ...
                    for l in range(len(jtem[1])):  # the longer name
                        ...
                        for mtem in existingAlignments:
                            if mtem[1] == jtem[1][l]:
                                ...
                                break
                        if (
                            jtem[1][l] not in unalignedWordIndicesInTheLongerName
                            or alreadyInserted
                        ):
                            continue
                        if (
                    ...
                    for l in range(len(item[1])):  # the longer name
                        ...
                        for mtem in existingAlignments:
                            if mtem[0] == item[1][k]:
                                ...
                                break
                        if (
                            item[1][l] not in unalignedWordIndicesInTheLongerName
                            or alreadyInserted
                        ):
                            continue
                        if (
