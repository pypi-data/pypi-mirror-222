from simpeval.aligner.config import *
            ...
            for j in startingIndicesForB:
                if a[i : i + size] == b[j : j + size]:
                    ...
                    for item in commonContiguousSublists:
                        if isSublist(currentAIndices, item[0]) and isSublist(
                            currentBIndices, item[1]
                        ):
                            alreadyInserted = True
                            break
                    if not alreadyInserted:
                        commonContiguousSublists.append(
                            [currentAIndices, currentBIndices]
                        )
...
def isAcronym(word, namedEntity):
    ...
    if (
        not canonicalWord.isupper()
        or len(canonicalWord) != len(namedEntity)
        or canonicalWord.lower() in ["a", "i"]
    ):
        return False
...
def group_sentence_alignments(sent1_parse_lst, sent2_parse_lst, sent_aligns):
    ...
    for index_pair in sent_aligns:
        ...
        if sent1_added and not sent2_added:  # it is a split
            ...
            if (
                len(sent1_parse[sent_group_index]) > 1
            ):  # check to prevent M-to-N alignments
            ...
            if (
                len(sent2_parse[sent_group_index]) > 1
            ):  # check to prevent M-to-N alignments
