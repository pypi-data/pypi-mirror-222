...
def loadPPDB(ppdb_file_name=PPDB_PATH):
    ...
    for entry in MWAPPDBCorpusReader(
        root="resources", fileids=ppdb_file_name
    ).entries():
...
def present_in_ppdb(word1, word2):
    ...
    return ((word1.lower(), word2.lower()) in ppdbDict) or (
        (word2.lower(), word1.lower()) in ppdbDict
    )
...
def wordRelatedness(word1, pos1, word2, pos2):
    ...
    if (
        canonical_word1.isdigit()
        and canonical_word2.isdigit()
