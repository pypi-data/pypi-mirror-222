...
from aviewpy.results import create_result
...
from .archard import wear_rate, instantaneous_wear_volume
...
class WearableShell():
    ...
    * J. F. Archard, “Contact and Rubbing of Flat Surfaces,” Journal of Applied Physics, vol. 
    ...
    def __init__(self, geom: GeometryShell):
        ...
        self.connections = [[np.argmin(np.linalg.norm(np.array(self.points) - np.array(geom.points[ip]), axis=1))
                             for ip in f]
                            for f in geom.connections]

        self.reference_marker = geom.reference_marker
        self.file_name = geom.file_name

        self.nominal_element_size = self._get_nominal_element_size()
    ...
    def _get_nominal_element_size(self, timeout=10):
        ...
        for facet in self.facets:
            ...
            if time.time() - st_time > timeout:
                n_sizes = len(dists_mag)
                print(f'_get_nominal_element_size timed out after {n_sizes} sizes')
                break

        return np.median(dists_mag)
