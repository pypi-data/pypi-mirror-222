...
def test_table_name(snapshot: Snapshot, make_snapshot: t.Callable):
    ...
    fully_qualified_snapshot = make_snapshot(SqlModel(name="catalog.db.table", query=parse_one("select 1, ds")))
    fully_qualified_snapshot.categorize_as(SnapshotChangeCategory.BREAKING)
    assert fully_qualified_snapshot.table_name(is_dev=False, for_read=False) == "catalog.sqlmesh__db.catalog__db__table__2528031000"
