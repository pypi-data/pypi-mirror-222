...
class ContextClient(KluClientBase):
    ...
    async def create(
        ...
        Args:
            ...
            loader (Optional[str]): The loader to be used on DataIndex.
                Can be one of: [gmail, notion, intercom, github, redis, crawler, database, elastic, zendesk, files, custom]
        ...
        if file_url:
            context["fileUrl"] = [file_url]  # type: ignore
    ...
    async def delete(self, guid: str) -> Context:
        ...
        Args:
            guid (str): The id of a context to delete.
    ...
    async def get_status(self, guid: int) -> TaskStatusEnum:
        """
        ...
        async with aiohttp.ClientSession() as session:
            client = self._get_api_client(session)
            try:
                response = await client.post(CONTEXT_STATUS_ENDPOINT.format(id=guid))
    ...
    async def process_context(
        self, context_guid: str, file_name: str, splitter: Optional[str] = None
    ) -> dict:
        """
        Processes existing index identified by provided context id using provider column splitter

        Args:
            context_guid (str): Guid of data index to process
            file_name (str): Name of the file uploaded before that should be used for Context processing.
    ...
    async def add_doc(
        ...
        Args:
            context_guid (str): Guid of the context which a document is created for
        ...
        async with aiohttp.ClientSession() as session:
            ...
            try:
                response = await client.post(
                    CONTEXT_DOCUMENTS_ENDPOINT.format(id=context_guid),
                    {
                        "text": text,
                        "filter": filter,
                        "meta_data": meta_data,
    ...
    async def list_docs(
        ...
        Args:
            context_guid (str): Guid of the context to fetch documents for

        Returns: response with new document guid and status
        """
        async with aiohttp.ClientSession() as session:
            client = self._get_api_client(session)
            try:
    ...
    async def delete_all_docs(
        ...
        Args:
            context_guid (str): Guid of the context to delete documents from
    ...
    async def update_doc(
        ...
        """
        Update single document on a context
