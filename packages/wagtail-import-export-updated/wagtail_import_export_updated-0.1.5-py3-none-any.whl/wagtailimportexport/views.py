...
from wagtailimportexport.compat import messages, Page, ngettext, gettext_lazy as _
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            import_url = base_url + reverse(
                "wagtailimportexport:export", args=[form.cleaned_data["source_page_id"]]
            )
...
def export_to_file(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            payload = export_pages(
                form.cleaned_data["root_page"], export_unpublished=True
            )
