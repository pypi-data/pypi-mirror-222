...
class BaseLabelme(metaclass=SingletonMeta):
    ...
    def model_to_iou(self, model_dataset, parameter):
        ...
        statistical_tb = pt.PrettyTable(['误检出', '漏检出', '背景图像', '正确检出', '图像精确率', '图像召回率', '标注正样本', '预测正样本', '图像准确率', 'F1分数'])
        image_title = '基于图像统计'
        ...
        statistical_tb.add_row(
            [error_check, leak_check, negative_sample, right_check, images_precision, images_recall, mark_positive, model_positive, images_accuracy, f1_score])
