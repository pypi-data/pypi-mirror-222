...
def manage(func):
    ...
    func_uuid = calculate_md5(func_bytes)
    func_source = inspect.getsource(func)

    resp = BaseClient.get(f"{Endpoints.HOST}/mariana/openapi/oss/getUdfStsToken?uuid={func_uuid}", {})
    if resp.status_code != 200:
        raise AIEError(10000000, f"注册{func.__name__}时遇到了错误: {resp.content.decode(encoding='utf-8')}")
    ...
    if not success:
        if result['code'] == 24240405:
            # udf 已经上传完成
            return func_uuid
        raise AIEError(10000000, f'注册{func.__name__}时遇到了错误: {result["message"]}')
    logger.info(f'Registering UDF {func.__name__}')
    ...
    with TqdmUpTo(desc=f"Registering Meta...") as t:
        bucket2.put_object(meta_file_key_sts['fileKey'], get_meta(func), progress_callback=t.update_to)
        t.total = t.n
