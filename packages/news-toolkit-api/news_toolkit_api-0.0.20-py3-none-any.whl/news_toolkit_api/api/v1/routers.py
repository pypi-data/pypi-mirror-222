...
async def get_feed(
    ...
    with client.context():
        ...
        feed_future = query.fetch_async(limit=limit, offset=offset)
        total_count_future = query.count_async()
        for feed in feed_future.get_result():
            feeds.append(
                FeedResponse(
                    id=feed.key.id(),
                    title=feed.title,
    ...
    return FeedsResponse(feed=feeds, total_count=total_count_future.get_result())
