...
from news_toolkit_api.config.settings import NEWS_TOOLKIT_REQUEST_TIMEOUT
...
class Repository(Generic[Content], metaclass=ABCMeta):
    ...
    @abstractmethod
    def parse_pagination(self, soup: BeautifulSoup, id: str) -> list[str]:
        pass
    ...
    async def fetch_content(self, id: str) -> tuple[Content | None, list[str]]:
        html_text = await self.fetch_html(self.build_url(id))
        if not html_text:
            return None, []
        ...
        return self.parse(soup, id), self.parse_pagination(soup, id)
