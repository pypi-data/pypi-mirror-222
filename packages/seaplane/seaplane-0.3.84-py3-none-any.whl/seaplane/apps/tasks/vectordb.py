...
from ...integrations.langchain import langchain_vectorstore, seaplane_embeddings, SeaplaneLLM
...
class Store:
    def __init__(self, index: str) -> None:
        ...
        vector_store.create_index(self.index)

    def save(self, file_name: str, file_url: str, embeddings: Embeddings = seaplane_embeddings) -> None:
        loader = PyPDFLoader(file_url)
        pages = loader.load_and_split()                
        texts = self.text_splitter.split_documents(pages)
        
        vectors = embeddings.embed_documents(texts)
        vectors = [Vector(vector=vector, metadata={"page_content": texts[idx].page_content, "metadata": texts[idx].metadata}) for idx, vector in enumerate(vectors)]
    ...
    def query(self, file_name: str, query: str, embeddings: Embeddings = seaplane_embeddings) -> Dict[str, Any]:
