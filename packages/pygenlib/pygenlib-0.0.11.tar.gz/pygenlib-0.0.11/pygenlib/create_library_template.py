...
def create_library_template(library_name, target_folder="."):
    ...
    # make a virtual environment in the target folder and activate it (check for windows or linux and use the appropriate command)
    if os.name == 'nt':
        os.system(
            f"cd {target_folder} && python -m venv venv && venv\\Scripts\\activate.bat")

    elif os.name == 'posix':
        os.system(
            f"cd {target_folder} && python -m venv venv && source venv/bin/activate")
    ...
    # Create the '__init__.py' file in the sub-library directory
    with open(os.path.join(sub_lib_path, '__init__.py'), 'w') as f:
        pass
    ...
    shutil.copy(os.path.join(templates_path, 'readme_template.md'),
                os.path.join(target_folder, 'README.md'))  # Changed from library_path to target_folder
    ...
    # Changed from library_path to target_folder
    with open(os.path.join(target_folder, 'setup.py'), 'w') as f:
    ...
    # Changed from library_path to target_folder
    with open(os.path.join(target_folder, 'updater.py'), 'w') as f:
