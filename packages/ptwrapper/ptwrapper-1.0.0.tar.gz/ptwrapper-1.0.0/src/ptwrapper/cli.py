...
def cli(test=False):
    ...
    parser = ArgumentParser(description='Simulate a PTR and generate the corresponding SPICE CK, resolved PTR, and quaternions dump file.')
    ...
    parser.add_argument("-p", "--ptr", help="Path to the PTR/PTX file.")
    ...
    if args.session_file:
        ...
        if not args.ptr:
            parser.error(f'PTR argument is missing')
        if not args.meta_kernel:
            parser.error(f'META_KERNEL argument is missing')
        try:
    ...
    if args.output_dir:
        ...
        print(f'Renaming quaternions.csv to {fname}.csv')
        sh.move(f'{root_scenario_path}/output/quaternions.csv', f'{output_dir}/{fname}.csv')
        ...
        remove_directory_if_empty(f'{root_scenario_path}/kernel')
        remove_directory_if_empty(f'{root_scenario_path}/output')
        remove_directory_if_empty(f'{root_scenario_path}/ptr')
        remove_directory_if_empty(f'{root_scenario_path}/config')
