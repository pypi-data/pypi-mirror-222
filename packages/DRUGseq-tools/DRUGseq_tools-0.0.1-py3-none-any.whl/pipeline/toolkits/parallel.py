...
import pipeline
from pipeline.toolkits.utils import find_assay_init, find_step_module
from pipeline.pipeline import ArgFormatter

TOOLS_DIR = os.path.dirname(pipeline.toolkits.__file__)
...
class Multi():
    ...
    def step_args(self):
        for step in self.__STEPS__:
            ...
            func_opts = getattr(step_module, f"get_para_{step}")
            func_opts(self.parser, optional=False)
...
'''
    ...
    def mapping(self, sample):
        step = 'mapping'
        fq = f'{self.outdir_dic[sample]["trim"]}/{sample}_clean_2.fq{self.fq_suffix}'
        cmd_line = self.get_cmd_line(step, sample)
        cmd = (
            f'{cmd_line} '
            f'--fq {fq} '
    ...
    def featureCounts(self, sample):
        step = 'featureCounts'
        input_bam = f'{self.outdir_dic[sample]["mapping"]}/{sample}_Aligned.sortedByCoord.out.bam'
        cmd_line = self.get_cmd_line(step, sample)
        cmd = (
            f'{cmd_line} '
            f'--input {input_bam} '
