...
def test_dataset_query_sample_chaining():
    ...
    new_weights = list()
    for i in range(0, len(dsv)):
        assert dsv.tensors[1][i].numpy()[0] == 1 or dsv.tensors[1][i].numpy()[0] == 2 or dsv.tensors[1][i].numpy()[0] == 3
...
def test_empty_dataset(tmp_datasets_dir):
    ds = deeplake.dataset(tmp_datasets_dir / "empty_ds", overwrite=True)
