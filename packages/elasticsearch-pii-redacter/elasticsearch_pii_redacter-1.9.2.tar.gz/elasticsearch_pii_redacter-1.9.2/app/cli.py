...
from app.defaults import TRACKING_INDEX
from app.helpers import check_logging_config, get_width, set_logging
...
@click.option('--logformat', help='Log output format', type=click.Choice(['default', 'ecs']))
@click.option('--tracking-index', help='Name for the tracking index', default=TRACKING_INDEX, type=str)
...
def run(
    ...
    dry_run, loglevel, logfile, logformat, redactions_file, tracking_index
):
    """CLI input"""
    ...
    main = Main(client_args, other_args, redactions_file, tracking_index, dry_run)
    main.run()
