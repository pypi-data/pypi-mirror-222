...
def parse_and_verify_body_data(required_params: set = None, required_signature: bool = True):
    def outer_wrapper(view_func):
        ...
        def wrapper(request: HttpRequest, *args, **kwargs):
            ...
            if not settings.DEBUG and required_signature:
                ...
                if request.data.get('vs'):  # TODO 废弃vs
...
def decrypt_and_verify_header_params(required_params: set = None, aes_header_key: str = None):
    def outer_wrapper(view_func):
        @functools.wraps(view_func)
        def wrapper(request: HttpRequest, *args, **kwargs):
