...
from core_devoops.db_filters import ServerSideFilter, SERVER_SIDE_FILTERS
...
def count_rows(fields: List[ServerSideField],
    ...
    with Session(engine) as session:
        return ceil(_get_full_query(fields, model, filter_str, session, True,
...
def _paginate_db_lines(fields: List[ServerSideField],
    ...
    Select relevant row lines from model db. Select the whole db if no limit or offset is provided.
    """
    query = _get_full_query(fields, model, filter_str, session, count=False,search_str=search_str,
                            search_cols=search_cols).order_by(*[field.field for field in fields])
...
def _get_filter_query(fields: List[ServerSideField],
    ...
    for key, (operator, value) in frontend_filters.items():
        field = first_or_default(fields, lambda x: x.col_name == key)
        query = SERVER_SIDE_FILTERS[field.col_name](query=query, operator=operator, value=value,
                                                    field=field.field)
