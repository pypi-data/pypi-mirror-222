...
对orm、db、sheet中的任意一个调用close( )方法即可。
...
1、关闭mysql连接后，orm、db、sheet都可以再使用。当再次使用时，ORM会调用mkconn方法创建一个新连接。

2、调用  orm.close( )、db.close( )、sheet.close( )  中的1个即可。若多次调用，ORM会创建一个新连接，然后再关闭这个新连接。

3、若长时间没有调用orm、db、sheet中的任意一个，mysql连接也会断开，这是mysql自身的机制。这不会影响这三者的后续使用，当再次调用它们时，ORM会自动重新连接mysql。
...
# 本文标签：编程、Python、MySQL、ORM、coolmysql
