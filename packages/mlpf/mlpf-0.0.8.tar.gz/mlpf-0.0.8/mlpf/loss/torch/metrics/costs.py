...
def calculate_cost(PQVA_matrix_prediction: Tensor, data: Data) -> Tensor:
    """
    ...
    active_powers_generation = (active_powers + data.active_power_demands) * data.baseMVA[data.batch]

    return torch.sum(polynomial_costs(active_powers_generation, data.cost_coefficients))
