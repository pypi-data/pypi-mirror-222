...
def cropandpaste_zarr(zarrpath, patchsize, numpatches, rescale, rescalefactor, gaussianblurstd=0.5, patcharray=""):
    ...
    if rescale:
        patcharray = np.zeros((int(patchsize * rescalefactor), int(patchsize * rescalefactor) * patchcount, normdata.shape[1]))
        blurredpatcharray = np.zeros((int(patchsize * rescalefactor), int(patchsize * rescalefactor) * patchcount, normdata.shape[1]))
    else:
