...
### TODO: Add volumetric imaging in the documentation.
...
class RAPIDGUI():
    ...
    def apply_clusters_defined_patches(self, patchesstart, isloadingmodel, outfolder, modelparams, markerindices,
                                       markernames, modelpath):
        """
        ...
        self.generate_RAPID_data(markerindices, markernames, os.path.join(outfolder, "RAPID_Data"), denoise,
        ...
        grey, prob, tab, colors, _ = runRAPIDzarr.train_rapid(args, device, os.path.join(outfolder, "RAPID_Data"),
                                                              outfolder, patchesstart)
        ...
        self.apply_pixel_clustering(tab.values, args, colors, selectimagesadded.grey, selectimagesadded.color)
            ...
            for edits in editactions:
                for j in imgnums:
                    if edits[j][i] == "Gaussian":
                        self.viewer.layers[i].data[j, :, :] = ndimage.gaussian_filter(self.viewer.layers[i].data[j, :, :], [1, 1])
                    elif edits[j][i] == "Median":
                        self.viewer.layers[i].data[j, :, :] = ndimage.median_filter(self.viewer.layers[i].data[j, :, :], [1, 1])
                    elif len(edits[j][i]) == 2:
                        self.viewer.layers[i].data[j, :, :] = self.apply_contrast_limits(self.viewer.layers[i].data[j, :, :], edits[j][i])
                if any([edits[j][i] == "Denoise" or edits[j][i] == "Binarize" for j in imgnums]):
                    ...
                    self.viewer.layers[i].data[denoiseimgnums, :, :] = np.moveaxis(denoise_img(np.moveaxis(self.viewer.layers[i].data[denoiseimgnums, :, :], 0, -1).astype(float)), -1, 0)
                if any([edits[j][i] == "Binarize" for j in imgnums]):
                    binarizeimgnums = [j for j in imgnums if edits[j][i] == "Binarize"]
                    self.viewer.layers[i].data[binarizeimgnums, :, :][self.viewer.layers[i].data[binarizeimgnums, :, :] > 0] = 255
            if not imgindex == -1:
                self.viewer.layers[i].data[imgindex, :, :] = self.edit_viewer.layers[i].data

    def apply_object_clustering(self, clusterids, tabindex, segmentedtab, outputpath, addcolorimg, addgreyimg, labelnames):
        """
        ...
        color = generate_colormap(numclusters + 1)
        ...
        for i in range(self.numimgs):
            ...
            relabeled = self.method_searchsort(np.arange(1, 1 + numcells), to_values, self.labeledimgs[i + analysisnum].flatten().astype(int))
        ...
        self.labeledimgs += [GUIUtils.convert_dtype(labelimg[i, :self.imageshapelist[i][0], :self.imageshapelist[i][1]]) for i in range(len(labelimg))]
        ...
        tabledata, my_data_scaled, distmatrix, uniqueclusters = \
            prep_for_mst(clustertable=data, minclustersize=1, clustersizes=data["# Cells"], outfolder=outputpath,
                         includedmarkers=paramslist)
        generate_mst(distancematrix=distmatrix, normalizeddf=my_data_scaled, colors=color, randomseed=0,
                     clusterheatmap=True, outfolder=outputpath, displaymarkers=paramslist,
                     uniqueclusters=uniqueclusters, samplenames=list(np.unique(data['Sample'])), displaysingle=False,
                     values="# Cells")

        # Update table sort module and other necessary variables.
        for i in range(self.numimgs):
            self.tableimagenames.append(f"Object Cluster {self.objectclustercount + 1} - {self.filenames[i].split('/')[-1]}")
    ...
    def apply_pixel_clustering(self, tab, args, colors, addgreyimg, addcolorimg):
        """
        ...
        for i in range(self.numimgs):
            labelimg = np.zeros((1, self.maximageshape[0], self.maximageshape[1]), dtype=self.labeledimgs[i-self.numimgs].dtype) - 1
    ...
    def apply_segmentation(self, zarrpath, probthreshold, minsize, maxsize, addgreyimg, addcolorimg, quantavg, outfolder):
        """
        Apply thresholds based on size and model prediction confidence to segmentation results.

        Args:
            zarrpath (str): Path to zarr file where results and image properties will be stored.
            probthreshold (float): Value in the range [0,1] defining model prediction probability threshold for cells to include.
            minsize (int): Minimum pixel area of cells to include in segmentation.
            maxsize (int): Maximum pixel area of cells to include in segmentation.
            ...
            quantavg (bool): If True, use mean expression values for quantification. Otherwise, calculate root-mean-square values.
            outfolder (str): Path to the folder where results will be saved.
        """
        ...
        for i in range(self.numimgs):
            ...
            blobs = np.array(fh[f"Features{i}"]) >= probthreshold
            ...
            expandobject = morphology.remove_small_objects(expandobject.astype(bool), min_size=int(minsize),
                                                           in_place=True)
            ...
            expandobject = expandobject[:self.imageshapelist[i][0], :self.imageshapelist[i][1]]
            expandobject = expandobject.astype(np.uint32)
            GUIUtils.save_img(os.path.join(outfolder, f"SegmentedLabels_{imgname}.tif"), expandobject, self.imageisflipped[i])
            self.labeledimgs.append(GUIUtils.convert_dtype(expandobject))

            # Store quantified expression values for each cell in an array.
            imgmean = np.zeros((objectcount, self.nummarkers + 4))
            for ch in range(self.nummarkers):
                data = self.viewer.layers[ch].data[i, :len(expandobject), :expandobject.shape[1]]
                if not quantavg:
                    data = data.astype(np.uint16)
                    data = data * data
                imgmean[:, ch] = measure.regionprops_table(expandobject, data, properties=['mean_intensity'])['mean_intensity']
                if not quantavg:
                    imgmean[:, ch] = np.sqrt(imgmean[:, ch])

            # Store quantified morphological values for each cell in the same array.
            intensityimg = copy.deepcopy(self.viewer.layers[0].data[i, :len(expandobject), :expandobject.shape[1]])
            imgmean[:, self.nummarkers] = [prop.area for prop in
                                           measure.regionprops(expandobject, intensity_image=intensityimg)]
            imgmean[:, self.nummarkers + 1] = [prop.eccentricity for prop in
                                               measure.regionprops(expandobject, intensity_image=intensityimg)]
            imgmean[:, self.nummarkers + 2] = [prop.perimeter for prop in
                                               measure.regionprops(expandobject, intensity_image=intensityimg)]
            imgmean[:, self.nummarkers + 3] = [prop.major_axis_length for prop in
                                               measure.regionprops(expandobject, intensity_image=intensityimg)]
            ...
            self.datalist.append(imgmean)
            numcells += len(imgmean)
            self.currentlyselected.append([])
            self.currenttableordersfiltered.append(list(range(len(imgmean))))
            ...
            GUIUtils.save_img(os.path.join(outfolder, f"Segmented_{imgname}.tif"),
                              rgbimage[:self.imageshapelist[i][0], :self.imageshapelist[i][1], :],
                              self.imageisflipped[i])
            ...
            for j in range(imgmean.shape[1]):
                minvals.append(np.min(imgmean[:, j]))
                maxvals.append(np.max(imgmean[:, j]))
        ...
        for i in range(self.numimgs):
            ...
            segmentedtable.columns = np.hstack([self.markers, "Area", "Eccentricity", "Perimeter", "Major Axis", "y", "x"])
    ...
    def biaxial_gate(self, segindex=None, chan1="", chan2="", colorparam="", norm="", colorbygroups=[],
                     colorbyindivclusters=None, colorbycombclusters=None, clusteringindex=None):
        """
        ...
        if any(param is None for param in (segindex, colorbyindivclusters, colorbycombclusters)) or any(param=="" for param in (norm, chan1, chan2, colorparam)):
            biaxial = GUIUtils.BiaxialGate(self.markers, self.objectimgnames, self.objectclustercount > 0, self.groupsnames[1:])
            biaxial.exec()
            if not biaxial.OK:
                return
        ...
        if len(self.segmentationclusteringrounds[segindex]) > 0:
            ...
            if colorbyindivclusters or colorbycombclusters:
                ...
                labelimg = self.concat_label_imgs([self.labeledimgs[ind] for ind in range(analysisnum, analysisnum+self.numimgs)])
    ...
    def change_folder(self, outputfolder=""):
        """
    ...
    def colormap_group(self, newcolorlist=[]):
        """
        ...
        if self.analysismode == "Pixel":
            ...
            labelimg = self.concat_label_imgs([self.labeledimgs[ind] for ind in range(analysisnum, analysisnum+self.numimgs)], pixelbased=True)
    ...
    def concat_label_imgs(self, imgs, pixelbased=False):
        dtype = np.uint32
        while True:
            for img in imgs:
                if img.dtype == dtype:
            ...
            for i, item in enumerate(data[key]):
                ...
                if i == 0:
                    ...
                    if self.analysismode == "Object" and numrows > 4:
                        ...
                        adj = ((data[key][i] - minv) / (maxv - minv) * self.maxpixelclustervals[clusterindex][j - 2]) / 255
                    else:
                        adj = 0.5
                    if math.isnan(adj):
                        adj = 0.5
    ...
    def display_selected_cells(self, plotindex=None, shapetypes=[], vertslist=[]):
        """
        ...
        for verts in vertslist:
            ...
            verts[:, 0] = ((self.plotxmaxs[plotindex] - verts[:, 0]) / (self.plotxmaxs[plotindex] - self.plotxmins[plotindex])) * 1.1 - 0.05
            verts[:, 1] = ((verts[:, 1] - self.plotymins[plotindex]) / (self.plotymaxs[plotindex] - self.plotymins[plotindex])) * 1.1 - 0.05
            verts[:, [0, 1]] = verts[:, [1, 0]]
        ...
        for shape in range(len(shapeverts)):
            ...
            for i in range(self.numimgs):
                ...
                labelimg = self.method_searchsort(np.unique(labelimg), np.array([j for j in range(len(np.unique(labelimg)))]), labelimg)
            ...
            if selectedcells == 0:
                ...
                for i in range(self.numimgs):
                    ...
                    for j in range(self.datalist[self.segmentationindices[segindex + i]].shape[1]):
                        try:
                            minvals.append(np.min(self.datalist[self.segmentationindices[segindex + i]][inclrows[i], j]))
                            maxvals.append(np.max(self.datalist[self.segmentationindices[segindex + i]][inclrows[i], j]))
                        except:
                            minvals.append(0)
                            maxvals.append(0)
    ...
    def draw_shapes(self, data, shape_type, properties, name, text, face_color):
    ...
    def edit_image(self, editactions=[]):
        """
    ...
    def filter_table(self, reset=None, bound="", marker="", val=None):
        """
        ...
        # If applying a new filter, add to the existing filters and update the table accordingly.
            ...
            if self.analysismode == "Segmentation":
                ...
                for i in range(len(self.lowerboundslist[self.tableindex])):
                    if self.lowerboundslist[self.tableindex][i] > self.minvals[self.tableindex][i] or self.upperboundslist[self.tableindex][i] < self.maxvals[self.tableindex][i]:
                ...
                filtereddata = np.append(self.datalist[self.tableindex][self.currenttableorderfull, :], np.expand_dims(np.arange(len(self.datalist[self.tableindex])), 1), 1)

                # Filter cells one marker at a time according to current lower- and upper-bounds.
                for markerid in filteredmarkers:
                    filtermask = (np.round(filtereddata[:, markerid], 3) <= np.round(self.upperboundslist[self.tableindex][markerid], 3))
                    filtereddata = filtereddata[filtermask]
                    filtermask = (np.round(filtereddata[:, markerid], 3) >= np.round(self.lowerboundslist[self.tableindex][markerid], 3))
                    filtereddata = filtereddata[filtermask]

                # Update the list of cell IDs included in the table for each image.
                self.currenttableordersfiltered[self.tableindex] = [self.currenttableorderfull[j] for j in filtereddata[:, -1].astype(np.int).tolist()]
                ...
                if len(self.datalist[self.tableindex]) > len(currentdata) > 0:
                    for i in range(self.numimgs):
                        if i == self.analysisindex % self.numimgs:
                            analysisnum = [j for j, n in enumerate(self.analysislog) if n == "S"][self.analysisindex//self.numimgs]
                            labelimg = self.labeledimgs[analysisnum*self.numimgs+i]
                            filtered = np.in1d(labelimg, np.asarray(self.currenttableordersfiltered[self.tableindex]) + 1)
                            ...
                            filtered = np.zeros((1, self.imageshapelist[self.analysisindex % self.numimgs][0],
                                                 self.imageshapelist[self.analysisindex % self.numimgs][1]), dtype=np.bool)
                        if i == 0:
                ...
                for i in range(len(self.datalist[self.tableindex])):
                    if i in self.currentlyselected[self.tableindex] and self.currenttableorderfull[i] not in self.currenttableordersfiltered[self.tableindex]:
            ...
            # If filtering an object clustering table.
                ...
                self.currenttableordersfiltered[self.tableindex] = [self.currenttableorderfull[i] for i in cData[:, -1].astype(np.int).tolist()]
                ...
                if len(self.datalist[self.tableindex]) > len(currentdata) > 0:
                    currentclusters = np.zeros((self.numimgs, self.maximageshape[0], self.maximageshape[1]), dtype=np.bool)
                    labelimg = self.concat_label_imgs([self.labeledimgs[ind] for ind in range(imganalysisnum*self.numimgs, imganalysisnum*self.numimgs+self.numimgs)])
                ...
                for i in range(len(currentdata)):
                    for j in range(ind, ind+numtabs):
                        if self.currenttableorderfull[i] in self.currentlyselected[j] and self.currenttableorderfull[i] not in self.currenttableordersfiltered[self.tableindex]:
            ...
            # If filtering a pixel clustering table.
                ...
                if len(self.datalist[self.tableindex]) > len(currentdata) > 0:
                    currentclusters = np.zeros((self.numimgs, self.maximageshape[0], self.maximageshape[1]), dtype=np.bool)
                    labelimg = self.concat_label_imgs([self.labeledimgs[ind] for ind in range(imganalysisnum*self.numimgs, imganalysisnum*self.numimgs+self.numimgs)], pixelbased=True)
                    for index in self.currenttableordersfiltered[self.tableindex]:
    ...
    def generate_RAPID_data(self, markerindices, markernames, outfolder, denoise, normalizeeach, normalizeall, normtype,
        ...
        for i in range(self.numimgs):
            ...
            vaex.from_pandas(pd.DataFrame(img).astype('float32')).export_hdf5(os.path.join(self.outputfolder, 'hdf5_files', (f'analysis_{i:02}.hdf5')))
    ...
    def load_environment(self, envpath=""):
        """
    ...
    def load_object_clusters(self, csvpath="", segindex=None, addgreyimg=None, addcolorimg=None):
        """
        ...
        for i in range(self.numimgs):
            ...
            relabeled = self.method_searchsort(from_values, tmp_to_values, self.labeledimgs[analysisnum + i].flatten().astype(int))
            relabeledgreyimages[i, :self.imageshapelist[i][0], :self.imageshapelist[i][1]] = relabeled.reshape((self.imageshapelist[i][0], self.imageshapelist[i][1])).astype(np.uint8)
            relabeledgreyimages[i, :self.imageshapelist[i][0], :self.imageshapelist[i][1]][self.labeledimgs[analysisnum + i] == 0] = 0
            ...
            if i == 0:
                ...
                relab[0, :len(relabeledimages), :relabeledimages.shape[1], :] = relabeledimages
                if addgreyimg and addcolorimg:
                    self.viewer.layers[-2].data = np.vstack((self.viewer.layers[-2].data, relabeledgreyimages[[i], :, :]))
                    self.viewer.layers[-1].data = np.vstack((self.viewer.layers[-1].data, relab))
                elif addgreyimg:
                    self.viewer.layers[-1].data = np.vstack((self.viewer.layers[-1].data, relabeledgreyimages[[i], :, :]))
                elif addcolorimg:
                    self.viewer.layers[-1].data = np.vstack((self.viewer.layers[-1].data, relab))
        ...
        self.labeledimgs += [GUIUtils.convert_dtype(relabeledgreyimages[i, :self.imageshapelist[i][0], :self.imageshapelist[i][1]]) for i in range(len(relabeledgreyimages))]
        ...
        tabledata, datascaled, DistMatrix, uniqueClusters = \
            prep_for_mst(clustertable=clusterdf, minclustersize=1, clustersizes=clusterdf["# Cells"],
                         outfolder=outfolder, includedmarkers=mergemarkerlist)
        generate_mst(distancematrix=DistMatrix, normalizeddf=datascaled[datascaled.columns], colors=color,
                     randomseed=0, outfolder=outfolder, clusterheatmap=True, displaymarkers=mergemarkerlist,
                     uniqueclusters=uniqueClusters, samplenames=list(np.unique(clusterdf['Sample'])),
                     displaysingle=False, values="# Cells")
    ...
    def load_pixel_results(self, datapath="", outputfolder=""):
        """
        ...
        self.analysismode = "Pixel"
        self.labeledimgs += [GUIUtils.convert_dtype(greyimgs[i, :self.imageshapelist[i][0], :self.imageshapelist[i][1]]) for i in range(len(greyimgs))]
        self.analysislog.append("O")
        ...
        self.apply_pixel_clustering(tab, args, c, selectimagesadded.grey, selectimagesadded.color)
    ...
    def load_segmentation_results(self, filenames=[], outputfolder=""):
        """
        ...
        for path in filenames:
            ...
            loadedimg, flipimg = self.parse_img(filename, True)
        ...
        for i, path in enumerate(filenames):
            ...
            minvals = []
            maxvals = []
            imgmean = np.zeros((len(np.unique(labelimages)) - 1, self.nummarkers + 4))
            for ch in range(self.nummarkers):
                imgmean[:, ch] = measure.regionprops_table(labelimages, self.viewer.layers[ch].data[i, :, :], properties=['mean_intensity'])['mean_intensity']
            imgmean[:, self.nummarkers] = [prop.area for prop in measure.regionprops(labelimages, intensity_image=self.viewer.layers[0].data[i, :, :])]
            imgmean[:, self.nummarkers + 1] = [prop.eccentricity for prop in measure.regionprops(labelimages, intensity_image=self.viewer.layers[0].data[i, :, :])]
            imgmean[:, self.nummarkers + 2] = [prop.perimeter for prop in measure.regionprops(labelimages, intensity_image=self.viewer.layers[0].data[i, :, :])]
            imgmean[:, self.nummarkers + 3] = [prop.major_axis_length for prop in measure.regionprops(labelimages, intensity_image=self.viewer.layers[0].data[i, :, :])]

            cortab = [prop.centroid for prop in measure.regionprops(labelimages, intensity_image=self.viewer.layers[0].data[i, :, :])]
            rgbimage = label2rgb(labelimages, image=None, colors=None, alpha=0.3, bg_label=0, bg_color=(0, 0, 0), image_alpha=1, kind='overlay')
            ...
            self.minvals.append(copy.deepcopy(minvals))
            self.maxvals.append(copy.deepcopy(maxvals))
            self.labeledimgs.append(GUIUtils.convert_dtype(labelimages[:self.imageshapelist[i][0], :self.imageshapelist[i][1]]))
        ...
        if 'None' in self.tableimagenames:
            self.tableimagenames.remove('None')
        self.currenttableorderfull = list(range(len(self.datalist[self.segmentationindices[0]])))
        print(self.numimgs)
        for i in range(self.numimgs):
            order = list(range(len(self.datalist[self.segmentationindices[i]])))
            self.currenttableordersfiltered.append(order)
        self.segmentationclusteringrounds.append([])
        self.updatelogfile = False
        self.sorttableimages.data.choices = tuple(self.tableimagenames)
        ...
        if not self.hasaddedtable:
            self.lowerboundslist[self.tableindex] = copy.deepcopy(self.minvals[0])
            self.upperboundslist[self.tableindex] = copy.deepcopy(self.maxvals[0])
            self.update_table(self.datalist[self.segmentationindices[0]],
                              self.lowerboundslist[self.tableindex],
                              self.upperboundslist[self.tableindex],
                              len(self.datalist[self.segmentationindices[0]]),
                              list(range(1, 1+len(self.datalist[self.segmentationindices[0]]))))
        return filenames

    def manual_annotation(self, umapind=None, addcolorimg=None, addgreyimg=None, labelnames=[], shapeverts=[], shapetypes=[]):
        """
        ...
        self.apply_object_clustering(clusterids, tabindex, segmentedtab, outputpath, addcolorimg, addgreyimg, labelnames)

        GUIUtils.log_actions(self.actionloggerpath, f"gui.manual_annotation(umapind={umapind}, "
                                                    f"addcolorimg={addcolorimg}, addgreyimg={addgreyimg}, "
                                                    f"labelnames={labelnames}, "
                                                    f"shapeverts={[verts.tolist() for verts in shapeverts]}, "
                                                    f"shapetypes={shapetypes})")

    def merge_clusters(self, checked=[], addcolorimg=None, addgreyimg=None):
        """
        ...
        GUIUtils.log_actions(self.actionloggerpath, f"gui.merge_clusters(checked={checked}, addcolorimg={addcolorimg}, "
                                                    f"addgreyimg={addgreyimg})")
        ...
        if self.analysismode == "Pixel":
            ...
            outfolder = GUIUtils.create_new_folder(os.path.join(os.path.split(self.pixelclusterdirectories[analysisnum])[-1], "Merged_"), self.outputfolder)
            ...
            imganalysisnum = [i for i, n in enumerate(self.analysislog) if n == "P"][self.analysisindex // numtabs] * self.numimgs
            labelimg = self.concat_label_imgs([self.labeledimgs[ind] for ind in range(imganalysisnum, imganalysisnum + self.numimgs)], pixelbased=True)
            ...
            clusterorder.append(newclusterval)
            clusterorderfull.append(newclusterval)
            ...
            for i in range(len(labelimg)):
                self.labeledimgs[imganalysisnum+i] = labelimg[i, :self.imageshapelist[i][0], :self.imageshapelist[i][1]].astype(self.labeledimgs[imganalysisnum+i].dtype)
            ...
            generate_mst(distancematrix=DistMatrix, normalizeddf=my_data_scaled[my_data_scaled.columns], colors=color,
                         randomseed=0, outfolder=outfolder, clusterheatmap=True, displaymarkers=markernames,
                         uniqueclusters=uniqueClusters, samplenames=samplenames, displaysingle=False)

            # Add image(s) to the viewer
            self.set_invisible(self.viewer)
            if addgreyimg:
                self.viewer.add_image(labelimg, name=f"Merged Pixel Cluster IDs {analysisnum}", blending="additive", contrast_limits=[0, np.max(labelimg)])
            if addcolorimg:
        ...
        # If the uer is merging object-based clusters.
            ...
            outfolder = GUIUtils.create_new_folder(os.path.join(os.path.split(self.objectclusterdirectories[analysisnum])[-1], "Merged_"), self.outputfolder)
            ...
            imganalysisnum = [i for i, n in enumerate(self.analysislog) if n == "O"][self.analysisindex // numtabs] * self.numimgs
            labelimg = self.concat_label_imgs([self.labeledimgs[ind] for ind in range(imganalysisnum, imganalysisnum + self.numimgs)])
            newclusterval = int(np.max(labelimg) + 1)
            clusterorder = self.currenttableordersfiltered[self.tableindex]
            clusterorderfull = self.currenttableorderfull
            if not self.clusternames[ind] == []:
                ...
                for cluster in checked:
                    ...
                    self.objectclusterdfs[analysisnum]['Cluster'][self.objectclusterdfs[analysisnum]['Cluster'] == oldname] = newname
                    cluster += 1
                    labelimg[labelimg == cluster] = newclusterval
                    labelimg[labelimg > cluster] = labelimg[labelimg > cluster] - 1
                    newclusterval -= 1
                    ...
                    for i in range(self.numimgs):
                        self.cellclustervals[imganalysisnum + i][self.cellclustervals[imganalysisnum + i] == cluster] = newclusterval + 1
                        self.cellclustervals[imganalysisnum + i][self.cellclustervals[imganalysisnum + i] > cluster] -= 1
                ...
                for cluster in checked:
                    ...
                    for i in range(self.numimgs):
                        self.cellclustervals[analysisnum * self.numimgs + i][self.cellclustervals[analysisnum * self.numimgs + i] == cluster] = newclusterval + 1
                        self.cellclustervals[analysisnum * self.numimgs + i][self.cellclustervals[analysisnum * self.numimgs + i] > cluster] -= 1
                    index = f"Cluster {cluster} (Object [{analysisnum}])"
                    for i in reversed(range(len(self.viewer.layers))):
                        if self.viewer.layers[i].name == index:
                            self.viewer.layers.pop(i)
            ...
            np.save(os.path.join(outfolder, "color.npy"), color)
            tabledata, my_data_scaled, distmatrix, uniqueclusters = \
                prep_for_mst(clustertable=my_data, minclustersize=1, clustersizes=my_data["# Cells"],
                             outfolder=outfolder, includedmarkers=paramslist)
            generate_mst(distancematrix=distmatrix, normalizeddf=my_data_scaled, colors=color, randomseed=0,
                         clusterheatmap=True, outfolder=outfolder, displaymarkers=paramslist,
                         uniqueclusters=uniqueclusters, samplenames=list(np.unique(my_data['Sample'])),
                         displaysingle=False, values="# Cells")
    ...
    def merge_markers(self, nucmarkernums=[], nucalg="", memmarkernums=[], memalg="", nuccls=[], memcls=[]):
        """
    ...
    def minimum_spanning_tree(self, clusteringindex=None):
        """
        ...
        clustermodeindex = [i for i, ispixelbased in enumerate(self.clustersarepixelbased) if ispixelbased == ispixelcluster].index(clusteringindex)
        ...
        self.set_invisible(self.viewer)
        self.viewer.add_image(arrays, name=f"MST (Pixel {clustermodeindex+1}" if ispixelcluster else f"Object {clustermodeindex+1})", blending="additive")
        GUIUtils.log_actions(self.actionloggerpath, f"gui.minimum_spanning_tree(clusteringindex={clusteringindex})")

    def nearest_neighbours(self, imgname="", clusteringindex=None, sourcecluster=None, targetcluster=None, radius=None,
                           numnn=None):
        """
        ...
        tableindex = self.objectclusterindices[clusteringround]
    ...
    ### TODO: Compare object_clustering with subcluster, see if any code can be consolidated
    def object_clustering(self, markernums=[], segindex=None, algname="", modelpath="", addgreyimg=None,
                          addcolorimg=None, continuetraining=None, normalize="", pca=False, modelparams=[]):
        """
        ...
        markernames = [params[i] for i in markernums]
        ...
        if algname == "Pretrained":
            ...
            args = runRAPIDzarr.get_parameters()
            if modelparams == []:
                setParams = GUIUtils.RAPIDObjectParameters(len(markernums))
                setParams.exec()
                if not setParams.OK:
                    return
        ...
        if args.normalize == "zscale":
            ...
            for i in range(self.numimgs):
                numcells = len(self.datalist[self.segmentationindices[i + imagenum]])
                expressionavgs[count:count + numcells, :] = self.datalist[self.segmentationindices[i + imagenum]][:, markernums]
                count += numcells
        ...
        GUIUtils.log_actions(self.actionloggerpath, f"gui.object_clustering(markernums={markernums}, segindex={segindex}, "
    ...
    def open_docs(self):
        ...
        if not os.path.exists(rootfold+"/../docs"):
            ...
            gdown.download("https://drive.google.com/uc?id=1rZ3pH9egK0g5Jxj-7vyoHX5meLxpmFYt",rootfold+"/../docs.zip", verify=False)
    ...
    def open_images(self, segmentedimgpaths=[], filenames=[], cfirst=None, markerstring="", loadedmatrix=None,
                    imagenames=[], indiceslist=[], markernums=[]):
        """
        ...
        if self.addwhenchecked and (column == 0 and row > 2) or row == 2:
            ...
            if currentState == QtCore.Qt.Checked:
                if column > 0:
                    ...
                    if self.analysismode == "Segmentation":
                        analysisnum = [i for i, n in enumerate(self.analysislog) if n == "S"][self.analysisindex // self.numimgs]
                        for i in range(self.numimgs):
                            cellimg = np.zeros((1, self.maximageshape[0], self.maximageshape[1]), dtype=np.bool)
                            if i == self.analysisindex % self.numimgs:
                                mask = np.in1d(self.labeledimgs[analysisnum * self.numimgs + self.analysisindex % self.numimgs], r+1)
                        ...
                        objectanalysisnum, numtabs = GUIUtils.find_analysis_round(self.analysisindex, self.numimgs)
                        color = self.objectclustercolors[objectanalysisnum][r, :] / 255
                        analysisnum = [i for i, n in enumerate(self.analysislog) if n == "O"][objectanalysisnum] * self.numimgs
                        cmap = Colormap(ColorArray([(0, 0, 0), (color[0], color[1], color[2])]))

                        for i in range(self.numimgs):
                            clusterimg = np.zeros((1, self.maximageshape[0], self.maximageshape[1]))
                            clusterimg[0, :self.imageshapelist[i][0], :self.imageshapelist[i][1]] = self.labeledimgs[analysisnum + i]
                            clusterimg = np.in1d(clusterimg, r + 1)
                            clusterimg = clusterimg.reshape((1, self.maximageshape[0], self.maximageshape[1]))
                            if i == 0:
                                self.viewer.add_image(clusterimg, name=f"Cluster {r + 1} (Object [{objectanalysisnum}])", blending="additive", colormap=cmap, visible=True)
                            else:
                        ...
                        pixelanalysisnum, numtabs = GUIUtils.find_analysis_round(self.analysisindex, self.numimgs)
                        color = self.pixelclustercolors[pixelanalysisnum][r, :] / 255
                        analysisnum = [i for i, n in enumerate(self.analysislog) if n == "P"][pixelanalysisnum] * self.numimgs
                        cmap = Colormap(ColorArray([(0, 0, 0), (color[0], color[1], color[2])]))

                        for i in range(self.numimgs):
                            clusterimg = np.zeros((1, self.maximageshape[0], self.maximageshape[1]))
                            clusterimg[0, :self.imageshapelist[i][0], :self.imageshapelist[i][1]] = self.labeledimgs[analysisnum + i] + 1
                            clusterimg = np.in1d(clusterimg, r + 1)
                            clusterimg = clusterimg.reshape((1, self.maximageshape[0], self.maximageshape[1]))
                            if i == 0:
                                self.viewer.add_image(clusterimg, name=f"Cluster {r + 1} (Pixel [{pixelanalysisnum}])", blending="additive", colormap=cmap, visible=True)
                            else:
                                self.viewer.layers[-1].data = np.vstack((self.viewer.layers[-1].data, clusterimg))
                ...
                if column > 0:
                    ...
                    if self.analysismode == "Segmentation":
                        ...
                        index = f"Cluster {r + 1} (Object [{analysisnum}])"
                    else:
                        analysisnum, numtabs = GUIUtils.find_analysis_round(self.analysisindex, self.numimgs)
                        pixelclusterindex = self.pixelclusterindices.index(self.tableindex)
                        ind = self.tableindex - pixelclusterindex % numtabs
                        for i in range(ind, ind + numtabs):
                            self.currentlyselected[i].remove(r)
                        index = f"Cluster {r + 1} (Pixel [{analysisnum}])"
    ...
    def pixel_clustering(self, isloadingmodel=None, predict=None, randompatchgeneration=None, markerindices=[],
                         modelparams=[], modelpath="", patchesstart=[]):
        """
        ...
        if not randompatchgeneration and patchesstart == []:
            ...
            def finish_pixelgui() -> Image:
                for i in range(len(self.define_patches_viewer.layers)):
                    # Loop through each shape within each shapes layer.
                    if isinstance(self.define_patches_viewer.layers[i], napari.layers.shapes.shapes.Shapes) and self.define_patches_viewer.layers[i].visible:
                        for shape in range(len(self.define_patches_viewer.layers[i].data)):
                            # Split each shape into 64x64 patches, adding padding as necessary.
            ...
            modelparams = self.apply_clusters_defined_patches(patchesstart, isloadingmodel, outfolder, modelparams,
                                                              markerindices, markernames, modelpath)
            GUIUtils.log_actions(self.actionloggerpath, f"gui.pixel_clustering(isloadingmodel={isloadingmodel}, "
                                                        f"predict={predict}, "
                                                        f"randompatchgeneration={randompatchgeneration}, "
        ...
        # If randomly generating patches.
            ...
            if isloadingmodel and predict:
                ...
                # Normalize data for RAPID input.
                self.viewer.status = "Generating RAPID data..."
                self.generate_RAPID_data(markerindices, markernames, datafolder, False, args.normalize,
                                         args.normalizeall, args.normtype, args.pca)
            ...
            # If training a model.
                ...
                self.viewer.status = "Generating RAPID data..."
                self.generate_RAPID_data(markerindices, markernames, datafolder, denoise, args.normalize,
                                         args.normalizeall, args.normtype, args.pca)
            ...
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            self.viewer.window._status_bar._toggle_activity_dock(True)
            grey, prob, tab, colors, _ = runRAPIDzarr.train_rapid(args, device, datafolder, outfolder)
            self.viewer.window._status_bar._toggle_activity_dock(False)
            if not self.hasaddedtable:
                self.analysismode = "Pixel"
            if not os.path.exists(args.rfold):
    ...
    def quantify_object_cluster_region(self, imgindex, shapetypes, clusteriteration, verts):
        """
    ...
    def quantify_pixel_cluster_region(self, imgindex, shapetypes, clusteriteration, verts):
        """
    ...
    def quantify_raw_img_region(self, imgindex, shapetypes, verts):
        """
    ...
    def quantify_region(self, israwimg=None, clusteringindex=None, shapeverts=[], shapetypes=[], imgnum=None, regionnames=[]):
        """
        ...
        # If using clustered results, find number of pixels/cells from each cluster within each shape.
            ...
            clusteringind = [i for i, m in enumerate(self.clustersarepixelbased) if m == ispixelcluster][clustermodeindex]
            ...
            if ispixelcluster:
                ...
                avgs, numrows, numpixels = self.quantify_pixel_cluster_region(imgnum, shapetypes, clustermodeindex, shapeverts)
                ...
                # Add labels for each of the regions for the saved csv file and to add to the shapes.
                outfolder = GUIUtils.create_new_folder("QuantifiedPixelRegion_", self.outputfolder)
                addnewshapeslayer = True
                if regionnames is None:
                    quantifypopup = GUIUtils.QuantifyRegionPopup(avgs, "pixel", numrows, self.markers, numpixels, outfolder,
                                                                 self.selectedregioncount, clusternames=clusternames)
                    quantifypopup.exec()
                    if quantifypopup.saved:
                        regionnames = list(quantifypopup.headernames)[1:]
                    addnewshapeslayer = quantifypopup.saved
                else:
                    self.save_quantified_region(avgs, "pixel", numrows, self.markers, numpixels, outfolder,
                                                self.selectedregioncount, clusternames=clusternames)
                ...
                # Find averages and number of pixels in each shape.
                avgs, numrows, numcells, celldata = self.quantify_object_cluster_region(imgnum, shapetypes,
                                                                                        clustermodeindex, shapeverts)
                ...
                addnewshapeslayer = True
                if regionnames is None:
                    quantifypopup = GUIUtils.QuantifyRegionPopup(avgs, "object", numrows, self.markers, numcells,
                                                                 outfolder, self.selectedregioncount, celldata=celldata,
                    ...
                    self.save_quantified_region(avgs, "object", numrows, self.markers, numcells, outfolder,
                                                self.selectedregioncount, celldata=celldata, clusternames=clusternames)
                ...
                if not regionnames == labels and addnewshapeslayer:
                    ...
                    textproperties = {'text': '{class}', 'anchor': 'center', 'size': 10, 'color': 'white', }
                    self.viewer.add_shapes(shapeverts, shape_type=shapetypes, edge_width=0,
                                           name=f"Quantified Regions {self.selectedregioncount}", properties=properties,
                                           text=textproperties, face_color=[np.array([0.2, 0.2, 0.2])])
                self.selectedregioncount += 1
    ...
    def rename_clusters(self, clusteringindex=None, newclusternames=[]):
        """
        ...
        # Find current names of clusters.
        currentnames = self.clusternames[clusteringindex]
        ...
        if not ispixelcluster:
            self.objectclusterdfs[clustermodeindex]['Cluster'] = [newclusternames[oldnames.index(name)] for name in self.objectclusterdfs[clustermodeindex]['Cluster']]
            self.objectclusterdfs[clustermodeindex].to_csv(os.path.join(self.objectclusterdirectories[clustermodeindex], "SegmentationClusterIDs.csv"))
        ...
        if index == clustermodeindex and ((self.analysismode == "Pixel" and ispixelcluster) or (self.analysismode == "Object" and not ispixelcluster)):
            self.currentverticalheaderlabels[3:] = [newclusternames[clusternum] for clusternum in self.currenttableordersfiltered[self.tableindex]]
    ...
    def sample_group(self, namelist={}, name=""):
        """
    ...
    def save_data(self, savedimg="", ispixelcluster=None, clusteringnum=None, filename=""):
        """
        ...
        if savedimg == "Visible Window":
            ...
            GUIUtils.save_visible_full(self.viewer, self.outputfolder, self.filenames, self.imageisflipped, self.imageshapelist)
            GUIUtils.log_actions(self.actionloggerpath, f"gui.save_data(savedimg=\"{savedimg}\")")
            ...
            grey = self.concat_label_imgs([self.labeledimgs[i] for i in range(analysisnum, analysisnum + self.numimgs)], pixelbased=ispixelcluster)
    ...
    def save_environment(self):
        ...
        for i in range(len(self.viewer.layers)):
            if isinstance(self.viewer.layers[i], napari.layers.shapes.shapes.Shapes):
                ...
                data.attrs["Properties"] = self.viewer.layers[i].properties["class"].tolist() if "class" in self.viewer.layers[i].properties else ""
    ...
    def save_quantified_region(self, avgs, imgtype, numrows, markernames, numregions, outfolder, selectedregioncount,
    ...
    def segment(self, modelindex=None, imageres=None, mergemarkerindex=None, addgreyimg=None, addcolorimg=None,
                quantavg=None, probthreshold=None, minsize=None, maxsize=None, histogramnormalize=None):
        """
        ...
        if self.segmentcounts[mergemarkerindex][modelindex] == -1 or self.histogramcounts[mergemarkerindex][histogramnormalize] == -1:
        ...
        if all(prob is not None for prob in (probthreshold, minsize, maxsize)):
            self.apply_segmentation(zarrpath, probthreshold, minsize, maxsize, addgreyimg, addcolorimg, quantavg, outfolder)
            ...
            self.segmentviewer.add_image(binarized[:self.imageshapelist[0][0], :self.imageshapelist[0][1]],
                                         name="Segmentation", blending="additive", colormap="red", contrast_limits=[0, 1])
            if self.mergenucmarkers[mergemarkerindex]:
                self.segmentviewer.add_image(nucimg[0, :self.imageshapelist[0][0], :self.imageshapelist[0][1]], name="Merged Nuclear Markers", blending="additive")
            if self.mergememmarkers[mergemarkerindex]:
                self.segmentviewer.add_image(memimg[0, :self.imageshapelist[0][0], :self.imageshapelist[0][1]], name="Merged Membrane Markers", blending="additive")
            ...
            def change_image_segmentgui(image: str):
                ...
                blobs = self.remove_large_objects(blobs, maxsize=int(self.maxsize))
                segmented[blobs == 0] = 0
                self.segmentviewer.layers["Segmentation"].data = segmented[:self.imageshapelist[self.currentimagenum][0], :self.imageshapelist[self.currentimagenum][1]]
                if self.mergenucmarkers[mergemarkerindex]:
                    self.segmentviewer.layers["Merged Nuclear Markers"].data = nucimg[self.currentimagenum, :self.imageshapelist[self.currentimagenum][0], :self.imageshapelist[self.currentimagenum][1]]
                if self.mergememmarkers[mergemarkerindex]:
                    self.segmentviewer.layers["Merged Membrane Markers"].data = memimg[self.currentimagenum, :self.imageshapelist[self.currentimagenum][0], :self.imageshapelist[self.currentimagenum][1]]

            # Apply filters for final segmented results.
            @magicgui(call_button="Finish")
            def finish_segmentgui() -> Image:
                self.apply_segmentation(self.segmentationzarrpaths[self.segmentcounts[mergemarkerindex][modelindex]],
                                        self.probthreshold, self.minsize, self.maxsize, addgreyimg, addcolorimg, quantavg,
                                        outfolder)
    ...
    def sort_table_image(self, data="", marker="", sort=""):
        """
        ...
        if (self.segmentcount > 0 or self.pixelclustercount > 0) and not self.isloadingenv:
            ...
            if index in self.segmentationindices:
                ...
                if m > 0:
                    self.currenttableorderfull = np.argsort(self.datalist[self.tableindex][:, m-1]).astype(np.int).tolist()
                else:
                ...
                for chan in range(len(self.lowerboundslist[self.tableindex])):
                    filtermask = (np.round(filtereddata[:, chan], 3) <= np.round(self.upperboundslist[self.tableindex][chan], 3))
                    filtereddata = filtereddata[filtermask]
                    filtermask = (np.round(filtereddata[:, chan], 3) >= np.round(self.lowerboundslist[self.tableindex][chan], 3))
                    filtereddata = filtereddata[filtermask]
                self.currenttableordersfiltered[self.tableindex] = [self.currenttableorderfull[j] for j in filtereddata[:, -1].astype(np.int).tolist()]
            ...
            # If displaying object-based clustering results.
                ...
                for chan in range(len(self.lowerboundslist[self.tableindex])):
                    ...
                    filtermask = (np.round(filtereddata[:, chan], 3) >= np.round(self.lowerboundslist[self.tableindex][chan], 3))
                    filtereddata = filtereddata[filtermask]
                self.currenttableordersfiltered[self.tableindex] = [self.currenttableorderfull[i] for i in filtereddata[:, -1].astype(np.int).tolist()]
    ...
    def spatial_analysis(self, clusteringindex=None, npix=None, nsim=None):
        """
        ...
        for i in range(self.numimgs):
            clusterimg = np.zeros((self.maximageshape[0], self.maximageshape[1]), dtype=self.labeledimgs[analysisnum + i].dtype)
            if i == 0:
                rclusters = np.zeros((self.maximageshape[0], self.maximageshape[1]), dtype=self.labeledimgs[analysisnum + i].dtype)
    ...
    def subcluster(self, segindex=None, clusteringindex=None, markernums=[], clusternum=None, algname="", modelpath="",
                   addgreyimg=None, addcolorimg=None, continuetraining=None, modelparams=[]):
        """
        ...
        outfolder = GUIUtils.create_new_folder(os.path.join(os.path.split(self.objectclusterdirectories[clusteringindex])[-1], "Subclustered_"), self.outputfolder)
    ...
    def test_object(self, model, quantifiedvals, args, markerindices, addcolorimg, addgreyimg, alg, tabindex=0,
    ...
    def test_object_subcluster(self, model, quantifiedvals, args, numcellsperimage, iteration, clusternum, outfolder,
                               segindex, objectclustersstartindex, markerindices, alg, cellids, addcolorimg, addgreyimg):
        """
        ...
        labelimg = self.concat_label_imgs([self.labeledimgs[ind] for ind in range(objanalysisnum, objanalysisnum + self.numimgs)])
        ...
        tabledata, my_data_scaled, distmatrix, uniqueclusters = \
            prep_for_mst(clustertable=my_data, minclustersize=1, clustersizes=my_data["# Cells"],
                         outfolder=outfolder, includedmarkers=paramslist)
        generate_mst(distancematrix=distmatrix, normalizeddf=my_data_scaled, colors=colors, randomseed=0,
                     clusterheatmap=True, outfolder=outfolder, displaymarkers=paramslist,
                     uniqueclusters=uniqueclusters, samplenames=list(np.unique(my_data['Sample'])),
                     displaysingle=False, values="# Cells")
        ...
        if addgreyimg:
            self.viewer.add_image(labelimg, name='Re-Clustered IDs', blending="additive", contrast_limits=(0, np.max(labelimg)))
    ...
    def umap_plot(self, paramindices=[], segindex=None, min_dist=None, n_neighbors=None, metric="",
                  colorbymarkers=None, colorbygroups=[], colorbyindivclusters=None, colorbycombclusters=None,
        ...
        for i in range(self.numimgs):
            cellsincurrimg = []
            numcells = len(self.datalist[self.segmentationindices[segindex * self.numimgs + i]])
            currentimage2[count:count + numcells, :] = self.datalist[self.segmentationindices[segindex * self.numimgs + i]]
            currentimage[count:count + numcells, :] = self.datalist[self.segmentationindices[segindex * self.numimgs + i]][:, paramindices]
    ...
    def testGUI(self, segmentationfilenames=[], envpath="", pixelresultspath="", outputfolder=""):
        """
        ...
        with napari.gui_qt():
            ...
            if outputfolder == "" and envpath == "":
                ...
                if segmentationfilenames != []:
                    self.load_segmentation_results(segmentationfilenames, outputfolder)
            ...
            GUIUtils.log_actions(self.actionloggerpath, "import RAPID")
            GUIUtils.log_actions(self.actionloggerpath, "from RAPID.GUI.RAPIDGUI import RAPIDGUI")
            GUIUtils.log_actions(self.actionloggerpath, "gui = RAPIDGUI()")
            GUIUtils.log_actions(self.actionloggerpath, f"gui.testGUI(segmentationfilenames={segmentationfilenames}, "
                                                        f"envpath=\"{envpath}\", pixelresultspath=\"{pixelresultspath}\", "
