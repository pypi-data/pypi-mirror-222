...
APPROVED_RESPONSE_CASES = [
    ...
    ['nest', 'keyword', 'next'],
    ['nex', 'keyword', 'next'],
    ['herder', 'keyword', 'harder'],
    ['hardier', 'keyword', 'harder'],
    ['easer', 'keyword', 'easier'],
    ['earier', 'keyword', 'easier'],
    ['dtop', 'keyword', 'stop'],
]
...
def extract_approved_responses_to_list(
    ...
    extracted_approved_responses = [(answer_dict[token.replace('.', '').replace('*','')], token.replace('.', '').replace('*','')) for token in tokenized_student_message if token.replace('.', '').replace('*','') in answer_dict]
