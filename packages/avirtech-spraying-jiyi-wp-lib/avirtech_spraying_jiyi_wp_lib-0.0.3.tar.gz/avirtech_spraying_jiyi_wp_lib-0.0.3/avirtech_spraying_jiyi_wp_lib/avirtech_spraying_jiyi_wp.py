...
class autocorrect:
    ...
    def process_all():
        ...
        root = tk.Tk()
        root.withdraw()
        messagebox.showinfo("","Lokasi data waypoint hasil AVIRPLANER (.waypoints) dan data DSM Bila ada(.tif) ?")
        lokasi_dsm_waypoint = filedialog.askdirectory()
        ...
        lokasi_olah_wp = os.path.join(lokasi_output, "pengolahan_wp")
        os.mkdir(lokasi_olah_wp)
        # MENGUBAH WAYPOINT HASIL AVIRPLANER JADI SHP
        ...
        def convert_to_csv(waypoint_data, output_file):
            df = pd.DataFrame(waypoint_data)
            df.to_csv(output_file, index=False)

        data_wp = []
        for file in os.listdir(lokasi_dsm_waypoint):
            if file.endswith('.waypoints'):
                data_wp.append(os.path.join(lokasi_dsm_waypoint, file))

        input_file_path = data_wp[0]
        waypoint_data = parse_waypoint_file(input_file_path)
        flattened_data = [line[0].split('\t') for line in waypoint_data]
        ...
        df['start_x'] = df['x']
        df['end_x'] = df.iloc[1:, :].loc[:, 'x']
        df['end_x2'] = df['end_x'].shift(periods=-1)
        df = df.drop(['end_x'], axis=1)

        df['start_y'] = df['y']
        df['end_y'] = df.iloc[1:, :].loc[:, 'y']
        df['end_y2'] = df['end_y'].shift(periods=-1)
        df = df.drop(['end_y'], axis=1)
        df1 = df.drop(df.index[len(df)-1])
        # print(df1)
        df1.to_csv(os.path.join(lokasi_olah_wp,'waypoint_ready_to_shp.csv'))


        data_csv_waypoint = []
        for file in os.listdir(lokasi_olah_wp):
            if file.endswith('.csv'):
                data_csv_waypoint.append(os.path.join(lokasi_olah_wp, file))

        arcpy.XYToLine_management(data_csv_waypoint[0], os.path.join(lokasi_olah_wp, "jalur_drone_wp.shp"), "start_x", "start_y", "end_x2", "end_y2", "0", "no", "GEOGCS['GCS_WGS_1984',DATUM['D_WGS_1984',SPHEROID['WGS_1984',6378137.0,298.257223563]],PRIMEM['Greenwich',0.0],UNIT['Degree',0.0174532925199433]];-400 -400 1000000000;-100000 10000;-100000 10000;8,98315284119522E-09;0,001;0,001;IsHighPrecision")
        # arcpy.AddField_management("jalur_drone_ready_to_inject", "Id", "DOUBLE", "", "", "", "", "NULLABLE", "NON_REQUIRED", "")
        arcpy.Dissolve_management("jalur_drone_wp", os.path.join(lokasi_output, "jalur_drone_ready_to_inject.shp"), "", "", "SINGLE_PART", "DISSOLVE_LINES")
        ...
        def flat_altitude():
            ...
            for file in os.listdir(lokasi_dsm_waypoint):
                if file.endswith('.shp'):
                    data_jalur_terbang.append(os.path.join(lokasi_dsm_waypoint, file))
        ...
        def dsm_altitude ():
            ...
            for file in os.listdir(lokasi_dsm_waypoint):
                if file.endswith('.tif'):
                    data_dsm.append(os.path.join(lokasi_dsm_waypoint, file))
            data_jalur_terbang = []
            for file in os.listdir(lokasi_dsm_waypoint):
                if file.endswith('.shp'):
                    data_jalur_terbang.append(os.path.join(lokasi_dsm_waypoint, file))
