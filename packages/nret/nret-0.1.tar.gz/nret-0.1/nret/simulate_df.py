...
def simulate_df (tts, tto, psi, input_dataframe,):
    ...
    for index, row in input_dataframe.iterrows():
        ...
        rho_canopy = run_prosail(n, cab, car, cbrown, cw, cm, lai, lidfa, hspot, psi, ant, prot, cbc, tts, tto, rsoil=np.full(2101, rsoil_value), psoil=np.full(2101, psoil_value))
        rho.append(rho_canopy)

        #calculate fAPAR for each set of parameters
        fapar_data.append(calculate_fapar(n, cab, car, cbrown, cw, cm, lai, lidfa, hspot, psi, ant, prot, cbc, tts, tto, rsoil = np.full(2101, rsoil_value), psoil = np.full(2101, psoil_value)))

        # Calculate the reflectance by applying the spectral response functions
        reflectance_data.append(np.sum(rho_canopy * srf_land, axis=-1))
