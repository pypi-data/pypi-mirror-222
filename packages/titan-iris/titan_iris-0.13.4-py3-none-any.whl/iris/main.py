...
def takeoff(
    ...
    hf_auth_token: str = typer.Option(
        None, "--hf-auth-token", "-t", help="The huggingface auth token to use for private models."
    ),
):
    """Takeoff a model.
    ...
    try:
        if is_infer:
            ...
            extra_env_vars = {"ACCESS_TOKEN": hf_auth_token}
            extra_env_vars["HOST_URL"] = f"http://localhost:{port}"
            sdk.takeoff(model_folder_path=model_folder_path, device=device, port=port, extra_env_vars=extra_env_vars)
    except Exception as e:
        print(e)
        raise typer.Abort()
...
def shutdown_cli():
    ...
    container_idx = int(
        typer.prompt(
            typer.style(
                "Enter the index of the container you want to stop and remove: ",
