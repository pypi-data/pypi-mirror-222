...
def pull_fastapi_image(
    ...
    container_name = model_folder_path.split("/")[-1] + f"-{shortuuid.uuid()}"
    ...
    rprint(f"Container [bold cyan]{container_name} [/bold cyan]started.")
    rprint("You can use the following command to check the logs:")
    rprint(f"\n[bold red]docker logs {container_name} -f[/bold red]\n")
...
def list_running_containers():
    ...
    Returns:
        list: a list of running containers.
