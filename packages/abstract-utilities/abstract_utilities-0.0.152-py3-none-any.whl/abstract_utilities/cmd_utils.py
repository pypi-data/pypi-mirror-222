...
def cmd_run(cmd,output_txt:str=None):
    if output_txt == None:
        output_txt=get_output_txt()
    # Clear the output file before running the command
    with open(get_output_txt(), 'w') as f:
        pass
    cmd += f' >> '+output_txt+'; echo END_OF_CMD >> '+output_txt  # Add the delimiter at the end of cmd
    print(cmd)
...
def pexpect_cmd_with_args(command, child_runs,output_txt:str=None):
    if output_txt == None:
        output_txt=get_output_txt()
