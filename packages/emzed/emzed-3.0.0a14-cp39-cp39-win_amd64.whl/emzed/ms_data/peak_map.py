...
class ImmutablePeakMap(DbBackedModel):
    ...
    def load(cls, path, *, target_db_file=None, overwrite=False):
        ...
        spectra, chromatograms, source = pyopenms.load_experment(encode(path))
...
def create_table(access_name="peakmap", conn=None):
    ...
    conn.execute(
        f"""CREATE TABLE {access_name}_chromatograms (
                        mz           FLOAT
                        , precursor_mz FLOAT
                        , rts          BLOB
                        , intensities  BLOB
...
def chromatogram(
    ...
    return np.array(rts, dtype=np.float64), np.array(intensities, dtype=np.float32)
