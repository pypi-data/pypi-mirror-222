...
class CarsFleetService:
    ...
    def add_insurances(self, insurances: list[Insurance]) -> list[int]:
        """
        Adds multiple Insurances into BasicInsurances table
        :param insurances: list of BasicInsurance objects that will be inserted (ids will be created by auto_increment mechanizm,
        so we can provide None as id
        :return: list of id of new rows in insurances table created using BasicInsurances data
        """
        return self.basic_insurances_repo.insert_many(insurances)

    def add_mots(self, mots: list[Mot]) -> list[int]:
        """
        Adds multiple Mots into basic_mots table
        :param mots: list of BasicMots objects that will be inserted (ids will be created by auto_increment mechanizm,
        so we can provide None as id
        :return: list of id of new rows in basic_mots table created using BasicMots data
        """
        return self.basic_mots_repo.insert_many(mots)
