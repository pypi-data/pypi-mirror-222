...
def parse(hub, functions: Dict[str, CloudSpecFunction], targets: List[str]):
    ...
    for function_name in targets:
        ...
        if function_data:
            ...
            if (
                function_name
                in ["present", "absent", "get", "create", "update", "delete"]
                and "resource_id"
                not in hub.cloudspec.parse.param.simple_map(function_data.params).keys()
            ):
                function_data.params["resource_id"] = CloudSpecParam(
                    name="resource_id",
            ...
            if (
                function_name
                in ["present", "absent", "get", "list", "create", "update", "delete"]
                and "name"
                not in hub.cloudspec.parse.param.simple_map(function_data.params).keys()
            ):
                function_data.params["name"] = CloudSpecParam(
                    name="name",
                    doc=f"Idem name of the resource.",
                    required=True if function_name in ["present", "absent"] else False,
                    target="kwargs",
                    target_type="mapping",
                    param_type="str",
                    exclude_from_example=True,
