...
def get_row_description(client, source_id, row):
    ...
    row.columns = [ColumnsDataRow(name=c.name, type=c.graphqlType.upper(), index=c.index, selected=False, mutable=True) for c in table.columns if c.graphqlType]
    return row
        ...
        if (key.isalnum() and len(key) == 1) or key == 'underscore':
            ...
            # Keystrokes IN the context of a panel, that depends on the panel
                ...
                if ctrl is self.all_tables:
                    ...
                    if row:
                        if not row.columns:
                            self.futures.append(self.executor.submit(get_row_description, self.client, self.app.meta.source_data.source_id, row))
