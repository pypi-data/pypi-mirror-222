...
def app(fileName, input_theorem, input_display_theorem, input_display_fitch, input_display_gentzen ):
    if not os.path.isfile(fileName):
        return "Arquivo não encontradao"
    f = open(fileName, 'r')

    input_proof = f.read()
    return check_proof(input_proof,input_theorem=input_theorem, display_theorem=input_display_theorem, display_fitch=input_display_fitch,display_gentzen=input_display_gentzen)
