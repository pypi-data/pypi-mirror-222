...
BundleCore = namedtuple(
    "BundleCore",
    "bundles register unregister ingest load clean update switch bundle_info add",
)
...
def _make_bundle_core():
    ...
    def update(name, end_date=None,  environ=os.environ):
        ...
        min_start = min([equity.end_date for equity in equities])
    ...
    return BundleCore(bundles, register, unregister, ingest, load, clean, update , switch , bundle_info , add )


bundles, register, unregister, ingest, load, clean , update , switch , bundle_info , add = _make_bundle_core()
