...
#返回省市区名称和地区编号
def prurar_code(gkle='all'):
    if gkle=='专业类省份':
        return {"河北": 13, "辽宁": 21,"浙江": 33,"山东": 37,"重庆": 50}
    if gkle=='专业组省份':
        return {"广东": 44, "福建": 35,"湖南": 43,"海南": 46,"江苏": 32,"湖北": 42,"天津": 12,"北京": 11,"上海": 31}

    return {"北京": 11, "天津": 12, "河北": 13, "山西": 14, "内蒙古": 15, "辽宁": 21, "吉林": 22, "黑龙江": 23,
           "上海": 31, "江苏": 32, "浙江": 33, "安徽和甘肃": 34, "福建": 35, "江西": 36, "山东": 37, "河南": 41, "湖北": 42,
...
class QueryScoreRank:
    ...
    def get_batch_xian(self):
        ...
        for batch_xian in batch_xians:
            ...
            key = f'{batch_xian[0]}_{convert_kl(batch_xian[1])}_{pc}'
    ...
    def rbx_rank(self,prurar,pc,kl):
        ...
        :return:返回批次线和批次线位次
        """
        batch_xian = self.batch_xian_dict[f'{prurar}_{kl}_{"专科" if "专科" in pc else "本科"}']
...
def exenla(major):
    return '、'.join([v for v in
             ['联合培养', '中外合作', '校企合作', '国家专项', '高职本科', '地方专项', '高校专项', '联合办学', '精准扶贫',
...
def getexcelth(rowdatas0,ht):
    ...
    logger.error(f'表头中未找到“{ht}”相关字段名称，可以将“{ht}”字段添加至表中或将相应字段修改为“{ht}”')
    sys.exit()
...
def read_excel(file_path,sheet_index=0):
    ...
    if not file_path.endswith('.xlsx'):
        file_paths=file_sorting(file_path)
...
#查找文件夹下所有文件路径及文件名称
def file_sorting(path,isrfp=False):
    """
    :param path:文件夹路径
...
def replmjna(major,batch):
    ...
    if '专科' in batch:
        if major_name in zkzygm_dict:
            return major.replace(major_name,zkzygm_dict[major_name],1)
    return major

#删除字符串首尾看不见的字符，将中文括号改为英文括号
def optstr(s):
