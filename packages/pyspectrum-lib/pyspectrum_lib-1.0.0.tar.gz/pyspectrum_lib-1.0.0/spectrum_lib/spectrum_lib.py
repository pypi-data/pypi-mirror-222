...
if sys.platform == "win32":
    ...
    if arc[0].__contains__("64"):
        _libname = pathlib.Path(__file__).parent.resolve() / "libs" / "spectrumlib-x64.dll"
    else:
        _libname = pathlib.Path(__file__).parent.resolve() / "libs" / "spectrumlib-x32.dll"
...
_spectrum_lib = ctypes.WinDLL(str(_libname))
...
class SpectrumMath:
    ...
    def push_data(self, samples: List[float]):
