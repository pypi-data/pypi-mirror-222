...
from .omps_service import OMPSSession
from .user_datapoints import UserDataPoint
from .batch_jobs import BatchJob
from .utils import constants
from .utils.utility import get_token, env_resolver, get_client_environ, validate_date, MOSException
...
class MOSSession:
    ...
    def __init__(self, client_id: str = None, client_secret: str = None):
        ...
        self.base_url = "https://" + env_dict["pcs_host"] + "/" + env_dict["pcs_service_name"] + "/" + env_dict[
            "pcs_version"] + "/"
        self.headers = {
        ...
        self.logger.debug(
            f"MOS session created with PCS base url : {self.base_url} and version : {env_dict['pcs_version']}")
    ...
    def upload_taxlot_portfolio(self, as_of_date, asset_id, portfolio_id='BasePortfolio',
        ...
        if taxlot_df is None:

            if taxlot_file_path is None:
                taxlot_file_path = os.path.join(path, 'resources/taxlots.json')
    ...
    def get_portfolio_taxlots(self, as_of_date, portfolio_id='BasePortfolio', snapshot_type='CLOSE'):
        ...
        validate_date(as_of_date)
        self.logger.debug(f"Fetching taxlots with portfolio id {portfolio_id} as of date {as_of_date} with snapshot_type {snapshot_type}")
        taxlot = self.omps_session.get_taxlots_asofdate(portfolio_id=portfolio_id, as_of_date=as_of_date, snapshotType=snapshot_type, instrumentResolutionStrategy='LocatePrimaryID')
        if not taxlot:
            self.logger.info(f"No taxlot available with portfolio id:{portfolio_id}, date:{as_of_date} and snapshot_type:{snapshot_type}")
        else:
            return taxlot
    ...
    def get_portfolio_positions(self, as_of_date, portfolio_id='BasePortfolio', snapshot_type='CLOSE'):
        ...
        validate_date(as_of_date)
        self.logger.debug(f"Fetching positions with portfolio id {portfolio_id} as of date {as_of_date} with snapshot_type {snapshot_type}")
        positions = self.omps_session.get_positions_asofdate(portfolio_id=portfolio_id, as_of_date=as_of_date, snapshotType=snapshot_type, instrumentResolutionStrategy='LocatePrimaryID')
        if not positions:
            self.logger.info(f"No positions available with portfolio id:{portfolio_id}, date:{as_of_date} and snapshot_type:{snapshot_type}")
        else:
            return positions
