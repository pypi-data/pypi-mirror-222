...
def remove_inner_cell(curr_cell, cells, horizontals_new, verticals_new):
    for i in range(len(cells) - 1, -1, -1):
        box = cells[i]
        if curr_cell['x0'] <= box['x0'] < box['x1'] <= curr_cell['x1'] and curr_cell['y0'] <= box['y0'] < box['y1'] <= \
                curr_cell['y1']:
            cells.pop(i)
            verticals_new.pop(i)
            horizontals_new.pop(i)


def to_dicts(line_coordinates):
    arr = []
    for coordinate in line_coordinates:
        arr.append({'x0': int(coordinate.x0),
                    'x1': int(coordinate.x1),
                    'y0': int(coordinate.y0),
                    'y1': int(coordinate.y1)})
    return arr


def calculate_row_col_idx(cell, row_idx, temp_table):
    if len(temp_table) <= row_idx:
        temp_table.append([])
    rowspan = cell['rowspan']
    if rowspan > 1:
        for i in range(1, rowspan):
            if len(temp_table[row_idx]) == row_idx:
                temp_table.append([''])
            else:
                temp_table[row_idx].append('')
    col_idx = len(temp_table[row_idx])
...
class Pile(object):
    ...
    def find_table_cells(self, verticals, horizontals):
        # 借助opencv计算单元格的坐标和跨行跨列值
        # 分辨率为72像素时，A4尺寸为595px * 842px
        # 分辨率为300像素时，A4尺寸为2479px * 3508px
        # 分辨率为120像素时，A4尺寸为1487px * 2105px
        height = 842  # your img height
        width = 595  # your img width
        img = np.zeros((height, width, 3), np.uint8)

        # draw the table
        self.draw_table(img, height, verticals, horizontals)
        ...
        horizontals_new = []
        # 迭代轮廓并计算边界矩形(x,y,w,h)
        # cv2.RETR_LIST获取的边框当中，最后一个边框是外边框，排除
        for i in range(0, len(contours) - 1):
            contour = contours[i]
            # bounding rectangle
            # 这里获取的box是内框线
            x, y, w, h = cv2.boundingRect(contour)
            cell = {'x0': x, 'y0': y, 'x1': x + w, 'y1': y + h}

            # 移除内嵌单元格
            remove_inner_cell(cell, cells, horizontals_new, verticals_new)

            cells.append(cell)
            verticals_new.append({'x0': x, 'y0': y, 'x1': x + w, 'y1': y + h})
            horizontals_new.append({'x0': x, 'y0': y + h, 'x1': x + w, 'y1': y})

        # 获取横线和竖线的位置值
        vertical_coor_max = int(np.max([int(line['x0']) for line in verticals]))
        horizontal_coor_min = int(np.min([int(line['y0']) for line in horizontals]))
        vertical_coor = self._calc_coordinates(verticals_new, 'x0', False) + [vertical_coor_max]
        horizontal_coor = self._calc_coordinates(horizontals_new, 'y0', True) + [horizontal_coor_min]
        ...
        for cell in cells:
            ...
            cell['colspan'] = self._calculate_span(left, right, vertical_coor)
            cell['rowspan'] = self._calculate_span(bottom, top, horizontal_coor)

        return cells, vertical_coor_max, horizontal_coor_min, vertical_coor, horizontal_coor

    def _gen_table_intermediate(self):
        verticals = to_dicts(self.verticals)
        horizontals = to_dicts(self.horizontals)
        cells, v_max, h_min, vertical_coor, horizontal_coor = self.find_table_cells(verticals, horizontals)
        # 将单元格按行列排序
        cells = sorted(cells, reverse=False, key=lambda x: (-x['y1'], x['x0']))
        ...
        for cell in cells:
            ...
            row_idx, col_idx = calculate_row_col_idx(cell, row_idx, temp_table)
            cell['row_idx'] = row_idx
            cell['col_idx'] = col_idx

            cell['texts'] = self._find_cell_texts(cell['x0'], cell['y1'], cell['x1'], cell['y0'])
            intermediate[row_idx].append(cell)

            if cell['x1'] + 5 > v_max and num_cols > 1 and num_rows > 1 and row_idx < num_rows:
                row_idx += 1
    ...
    def _calc_coordinates(self, axes, attr, reverse):
        coor_set = set()
        for axis in axes:
            coor_set.add(axis[attr])
        coor_list = list(coor_set)
        coor_list.sort(reverse=reverse)
        return coor_list

    def _calculate_span(self, min, max, range_):
        assert len(range_) > 0
        count = 0
        for val in range_:
            if min + 2 < val < max - 2:
    ...
    def draw_table(self, img, height, verticals, horizontals):
        # draw the vertical lines
        for line in verticals:
            pt_start = (int(line['x0']), int(line['y1']))
            pt_end = (int(line['x1']), int(line['y0']))
            cv2.rectangle(img, pt_start, pt_end, (0, 255, 0), 1, 4)
