...
class Writer(object):
    ...
    def get_location(self):
        if self._mode == 'simple':
            return self._output + os.sep + self._title + '.md'
        elif self._mode == 'gitbook':
            return self._output + os.sep + self._title
        else:
            raise Exception('Unsupported mode: ' + self._mode)

    def _write_simple(self, piles):
        filename = self._output + os.sep + self._title + '.md'
    ...
    def _write_gitbook_from_intermediate(self, intermediate):
        book_dirname = self._output + os.sep + self._title
    ...
    def _write_gitbook_summary(self, book_dirname, intermediate):
        lines = []
        assert 'title' in intermediate, 'gitbook模式下，PDF需能解析出一级标题'
