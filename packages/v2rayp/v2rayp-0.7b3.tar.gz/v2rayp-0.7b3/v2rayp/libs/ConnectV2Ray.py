...
class ConnectV2Ray:
    ...
    def cmd_generator(
        ...
        if inside_windows():
            cmd = f"bin\\win\\v2ray\\xray.exe --config={self.config_file_path}"
        else:
            if platform.system() == "Darwin":
                cmd = f"bin/mac/v2ray/xray --config={self.config_file_path}"
            elif platform.system() == "Linux":
                cmd = f"bin/linux/v2ray/xray --config={self.config_file_path}"
