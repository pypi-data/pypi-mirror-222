    ...
    def start_window(self):
        ...
        while True:
            ...
            ##############################3
                ...
                if not inside_windows():
                    gost_about_path = gost_about_path.replace("\\", "/").replace(
                        ".exe", ""
                    )
                print(gost_about_path)
                xray_about_path = f"{config_path()}\\bin\\xray.exe"
                if not inside_windows():
                    xray_about_path = xray_about_path.replace("\\", "/").replace(
                        ".exe", ""
                    )
                string_about = ""
                if os.path.isfile(gost_about_path):
                    string = f"Gost version is:\t\t{os.popen(gost_about_path+' -V').read().strip()}\n\n"
                    string_about += string

                if os.path.isfile(xray_about_path):
                    string = f"Xray version is:\t\t{os.popen(xray_about_path + ' version').read().strip()}\n\n"
                    string_about += string
            ...
            ###################################
                ...
                if inside_windows():
                    ...
                    with open(path, "w") as file:
                        file.write(f"start /min {sys.executable} -m v2rayp")
                else:
                    path = "~/Desktop/v2rayp.sh"
                    with open(path, "w") as file:
