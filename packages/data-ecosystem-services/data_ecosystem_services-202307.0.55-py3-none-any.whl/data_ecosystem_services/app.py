...
cdc_files_protected_bp = Blueprint('protectected_files', __name__,
                                   url_prefix='/protectected_files')
...
def azure_ad_authentication(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            # Validate the access token using Azure AD
            validated_token = azure_credential.validate_token(access_token)

            # If the token is valid, call the original function
            return func(*args, **kwargs)
        except Exception:
            # Token validation failed or no token provided
...
class MetadataExcelFileUpload(Resource):
    ...
    def post(self):
        ...
        with tracer.start_as_current_span(f"metadata_excel_file_upload"):
            try:
