...
from .manifest_json import ManifestJson
...
import data_ecosystem_services.alation_service.manifest_excel as alation_manifest_excel
...
class Schema:
    ...
    def download_schema_manifest_excel_file(cls, alation_schema_id, config, schema_file_path):
        ...
        with tracer.start_as_current_span("download_schema_manifest"):
            ...
            try:
                ...
                df_schema, df_table_list, manifest_excel_file = manifest_excel.generate_excel_file_data(
        ...
        with tracer.start_as_current_span("download_schema_manifest"):
            try:
                ...
                metadata = manifest.validate_manifest(
                    manifest_json_file, schema_file_path)
    ...
    @classmethod
    def upload_schema_manifest_excel_file(cls, metadata_excel_data, config):
        """
        Uploads a schema manifest to Alation.
        ...
        with tracer.start_as_current_span("upload_schema_manifest"):
            try:
                ...
                schema_file_path = config.get("edc_json_schema_location")
                alation_user_id = config.get("edc_alation_user_id")
                alation_schema_id = metadata_excel_data.get("alationSchemaID")
                ...
                # Write the manifest file
                with open(manifest_json_file, "w", encoding='utf-8') as f:
                    json.dump(metadata_excel_data, f)
