...
def iframe() -> str:
    """Page that will automatically force an form upload of an image ot provoke challenge"""
    cfdata = get_challenge()
...
def problem() -> str:
    ...
    try:
        sendmail(
            f'Cloudflare challenge occured for "{url}" on server={request.root_url} ({platform.node()})',
        )
...
class Challenge:
    ...
    image_filename: str  # image filename for `url_for('static', filename=image_filename)`
