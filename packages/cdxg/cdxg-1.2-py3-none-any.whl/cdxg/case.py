...
from cdxg.webdriver import WebDriver
from cdxg.appdriver import AppDriver
from cdxg.request import HttpRequest, ResponseResult, formatting
...
from cdxg.utils import diff_json, AssertInfo, jmespath
...
class TestCase(unittest.TestCase, WebDriver, AppDriver, HttpRequest):
    ...
    def setUp(self):
        ...
        if (Cdxg.app_server is not None) and (Cdxg.app_info is not None):
            Cdxg.driver = Remote(Cdxg.app_server, Cdxg.app_info)
        self.start()
    ...
    def assertTitle(self, title: str = None, msg: str = None) -> None:
        ...
        for _ in range(Cdxg.timeout + 1):
            try:
                self.assertEqual(title, Cdxg.driver.title)
                break
            except AssertionError:
                sleep(1)
        else:
            self.assertEqual(title, Cdxg.driver.title, msg=msg)
    ...
    def assertInTitle(self, title: str = None, msg: str = None) -> None:
        ...
        for _ in range(Cdxg.timeout + 1):
            try:
                self.assertIn(title, Cdxg.driver.title)
                break
            except AssertionError:
                sleep(1)
        else:
            self.assertIn(title, Cdxg.driver.title, msg=msg)
    ...
    def assertUrl(self, url: str = None, msg: str = None) -> None:
        ...
        current_url = unquote(Cdxg.driver.current_url)
        for _ in range(Cdxg.timeout + 1):
            try:
    ...
    def assertInUrl(self, url: str = None, msg: str = None) -> None:
        ...
        for _ in range(Cdxg.timeout + 1):
            ...
            self.assertIn(url, Cdxg.driver.current_url, msg=msg)
    ...
    def assertText(self, text: str = None, msg: str = None) -> None:
        ...
        elem = Cdxg.driver.find_element(By.TAG_NAME, "html")
    ...
    def assertAlertText(self, text: str = None, msg: str = None) -> None:
        ...
        alert_text = Cdxg.driver.switch_to.alert.text
        for _ in range(Cdxg.timeout + 1):
            try:
