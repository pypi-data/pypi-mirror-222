...
class CollectPypiPlugin(SerialCollectorPlugin):
    ...
    def collect(self, comp: Component, repo_url: str, creds: CredentialRequiredService | None = None) -> Result:
        ...
        self.get_logger().info(f"Pypi collection type: {collection_type}", indent_level=2)
        self.get_logger().info(f"Attempting {type_order[0]} collection", indent_level=2)
        ...
        if run_result.returncode == 0:
            ...
            if not collection_type.startswith("both"):
                self.get_logger().debug(
                    f"Collection of {type_order[0]} successful, no {type_order[1]} collection needed", indent_level=2
                )
            ...
            self.get_logger().debug(
                f"Collection of {type_order[0]} failed, no {type_order[1]} collection needed", indent_level=2
            )
            return Result.retry(f"{base_error_msg}. Unable to collect {type_order[0]}", comp)

        self.get_logger().info(f"Attempting {type_order[1]} collection", indent_level=2)
    ...
    @staticmethod
    def _success_result(collection_type, type_order, success_count, last_rc, comp: Component) -> Result:
        message = ""
        match collection_type:
    ...
    @staticmethod
    def _retry_result(collection_type, type_order, success_count, base_error_msg, comp: Component) -> Result:
        message = ""
        match collection_type:
