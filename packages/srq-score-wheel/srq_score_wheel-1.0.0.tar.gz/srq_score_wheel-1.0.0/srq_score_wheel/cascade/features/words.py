...
def create_meta(data, _):
    ...
    data[mod.ACTIVITY_TYPE_MODEL] = np.where(
        data[mod.ACTIVITY_TYPE_MODEL] == "completion", 1, 0)
    data[mod.CONTENT_MODEL] = data[mod.CONTENT_MODEL].replace("'", '')
    data[mod.GOAL_NOTES_MODEL] = data[mod.GOAL_NOTES_MODEL].fillna("a")
    data["goal_notes_char_count"] = count_characters(data[mod.GOAL_NOTES_MODEL])
    data["char_count"] = data[mod.CONTENT_MODEL].apply(len)
    data["comma_count"] = count_punctuations(data[mod.CONTENT_MODEL], ",")
    data["stop_count"] = count_punctuations(data[mod.CONTENT_MODEL], ".")
    data["exclaim_count"] = count_punctuations(data[mod.CONTENT_MODEL], "!")
    data["word_count"] = count_characters(data[mod.CONTENT_MODEL])
    data['punctuation_count'] = data[mod.CONTENT_MODEL].apply(
        lambda x: len("".join(_ for _ in x if _ in string.punctuation)))

    return data, mod.META_FEATURE_NAMES
...
def create_pos_tag(data, feature_dicts):
    ...
    texts = data[mod.CONTENT_MODEL].tolist()
...
def check_pos_tag(text, flag):
    ...
    pos_family = pickle.load(open(f"{mod.PROD_MODEL_PATH_1}/pos_family", 'rb'))
    cnt = 0
    try:
...
def combine_topic_features(data, flags, nlp_dict):
    ...
    for key in ["lda_model", "id2word"]:
        object_path = f"{mod.PROD_MODEL_PATH_1}/{key}"
...
def get_feature_counts(data, feature_dict):
    return data[mod.CONTENT_MODEL].str.lower().str.count(r'|'.join(feature_dict))