...
def get_all_features(agg_data_train, feature_dicts, train_flag=0):
    ...
    return combine_word_features(
        ...
        feature_dicts,
        features_to_drop=mod.FEATURES_TO_DROP,
...
def pred_xgb(agg_data_pred_orig, xgb_models):
    ...
    xgb_word_embed_features = transform_df(agg_data_pred_orig[mod.CONTENT_MODEL].copy(deep=False))
    xgb_features = append_columns(agg_data_pred_orig[mod.XGB_TO_KEEP_FEATURES], xgb_word_embed_features)
...
def pred_svm(agg_data_pred_orig, svm_models):
    ...
    agg_data_pred_orig.loc[:, 'content_edit'] = extract_edited_content(agg_data_pred_orig[mod.CONTENT_MODEL])
...
def pred_knn(knn_prediction_features, knn_models):
    ...
    prob_predictions = make_predictions(knn_models, knn_data[mod.KNN_TO_KEEP_FEATURES], model_type=model_type)
...
def pred_rf(rf_prediction_features, rf_models):
    ...
    prob_predictions = make_predictions(rf_models, rf_data[mod.RF_TO_KEEP_FEATURES], model_type=model_type)
