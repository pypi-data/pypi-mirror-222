...
from hestia_earth.aggregation.utils.quality_score import calculate_score, has_min_score
from .utils import (
    AGGREGATION_KEYS,
...
def aggregate_country(country: dict, product: dict, cycles: list, source: dict, start_year: int, end_year: int) -> list:
    ...
    return list(filter(has_min_score, map(calculate_score, cycles + country_cycles)))
...
def aggregate_global(country: dict, product: dict, cycles: list, source: dict, start_year: int, end_year: int) -> list:
    ...
    return list(filter(has_min_score, [calculate_score(cycle, countries) for cycle in cycles]))
