...
def format_call(
    ...
    for name, param in sig.parameters.items():
        if name == "hub":
            ...
            # If param is a dataclass, get default values to populate if needed
            param_dataclass_defaults = (
                hub.tool.idem.arg_utils.get_defaults_if_dataclass(param)
            )
            if param_dataclass_defaults:
                dataclass_args.update({name: param_dataclass_defaults})
    ...
    for key in kwargs:
        try:
            ...
            # Populate default values for dataclasses
            hub.tool.idem.arg_utils.populate_default_values(
                dataclass_args.get(key), val
            )
