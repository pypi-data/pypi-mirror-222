...
- Quick and dirty tools, will get a proper river objects at some points but I need that now
...
py::dict RiverNetwork(fT threshold, py::array_t<fT,1>& tAQw, py::array_t<fT,1>& ttopo, Connector_t& connector, Graph_t& graph){
	...
	// River nodes
	std::vector<int> riverNode; riverNode.reserve(std::floor(connector.nxy()/100));

	// Step 1: calculate basin ID
	basid = graph._get_SFD_basin_labels();

	std::vector<fT> distfromoutlet(connector.nxy(),0.); graph._get_SFD_distance_from_outlets(distfromoutlet);
