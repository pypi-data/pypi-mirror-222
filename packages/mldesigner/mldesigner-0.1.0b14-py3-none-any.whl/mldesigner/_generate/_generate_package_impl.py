...
class AssetsUtil:
    ...
    def parse_validate_asset_dict(cls, asset_dict: PackageAssets) -> Tuple[dict, dict, list]:
        ...
        for module_path, assets in asset_dict.components.items():
            ...
            with tqdm(
                total=len(assets), desc="Listing components with '{}'".format(module_path), unit="component", position=0
...
def _generate_impl(source, working_dir, package_name, force_regenerate):
    ...
    with TimerContext() as timer:
        if num_processes > 1 and len(source.components) > 1:
            ...
            warning_summary, error_summary = _generate_impl(
                source=source, working_dir=working_dir, package_name=package_name, force_regenerate=force_regenerate
            )
