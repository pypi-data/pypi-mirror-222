...
from fmsfdata20.stream_parser.types import FilteredValue
...
def filter_caller(
    func: Callable, event: ParseEvent, exception: Exception = None, **kwargs
):
    argspec = inspect.getfullargspec(func)

    event_arg = next(filter(lambda x: x.startswith("ev"), argspec.args), "event")
    exception_arg = next(
        filter(lambda x: x.startswith("ex"), argspec.args), "exception"
    )
