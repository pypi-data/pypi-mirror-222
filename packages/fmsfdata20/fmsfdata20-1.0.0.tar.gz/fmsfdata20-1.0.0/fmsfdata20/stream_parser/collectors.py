...
from fmsfdata20.stream_parser.checks import (
    EventCheck,
    and_check,
    property_check,
    type_check,
)
from fmsfdata20.stream_parser.events import ParseEvent
from fmsfdata20.stream_parser.filters.generic import EventFilter, pass_event, until_match
from fmsfdata20.stream_parser.function_helpers import FunctionCaller, event_or_iterable
from fmsfdata20.stream_parser.stream import first_then_rest
...
def __collector(
    ...
    def wrapper(*args, **kwargs):
        ...
        while stream:
            ...
            if event is None:
                return
            collected_stream = peekable(
                first_then_rest(event, until_match(stream, end_check, yield_final=True))
            )
            if receive_stream:
