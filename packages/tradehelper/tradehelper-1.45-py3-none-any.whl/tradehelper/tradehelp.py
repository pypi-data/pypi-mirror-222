...
class utilities:
        ...
        try:
            if 'strategy_name' in args:
                ...
                response = requests.post(
                    self.base_url+"entry", headers=self.headers, data=data,timeout = 15)
    ...
    def exit(self, args, execution_id, exit_time, order_id, ltp, remarks, profit, loss, mtm, mtm_high, mtm_low):
        try:
            ...
            response = requests.post(
                self.base_url+"exit", headers=self.headers, data=data,timeout = 15)
            response_json = (response.json())
    ...
    def execution(self, args):
        try:
            ...
            if 'strategy_name' in args:
                ...
                print(data)
                response = requests.post(
                    self.base_url+"execution", headers=self.headers, data=data,timeout = 15)
                response_json = (response.json())
                # print(response_json)
    ...
    def log_update(self, args, mtm, high, low):
        try:
            if 'daily_execution_id' in args:
                ...
                response = requests.post(
                    self.base_url+"update", headers=self.headers, data=data,timeout = 15)
                response_json = (response.json())
        except Exception as e:
            print(e)
