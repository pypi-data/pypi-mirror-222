...
class AssertsContentMessagemanager(SqlsContentMessagemanager):
    ...
    def assert_content_messagemanager_page(self, _assert=True, **kwargs):
        ...
        # flag = self.compare_json_list(self.res, out, ["content", "messageId", "cStartTime", "authId", "pStartTime", "cEndTime", "endTime", "userId", "beginTime", "pEndTime", "status"])
        assert True, "数据比较不一致"
    ...
    def assert_content_messagemanager_insert(self, _assert=True, **kwargs):
        ...
        # flag = self.compare_json_list(self.res, out, ["param", "content", "messageId", "checkStatus", "imageUrl", "type", "publishTime", "sourceType", "sendScope", "publishType", "checkPublish"])
        assert True, "数据比较不一致"
    ...
    def assert_content_messagemanager_update(self, _assert=True, **kwargs):
        ...
        # flag = self.compare_json_list(self.res, out, ["userMobiles", "param", "content", "messageId", "checkStatus", "imageUrl", "type", "sourceType", "sendScope", "checkPublish"])
        assert True, "数据比较不一致"
