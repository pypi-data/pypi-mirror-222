...
class TestPage:
    ...
    def test_page__districtNameSort_007(self):
    ...
    def test_page__shopBusinessType_012(self):
        self.f.page(shopBusinessType=True)

    # @pytest.mark.skip("不执行")
    @allure.step(title="网点列表-获取经销商__单参数有值： dealerCode")
    def test_page__dealerCode_013(self):
        self.f.page(dealerCode=True)

    # @pytest.mark.skip("不执行")
    @allure.step(title="网点列表-获取经销商__所有参数有值")
    def test_page__all_014(self):
        self.f.page(city=True, district=True, cityNameSort=True, districtNameSort=True, dealerName=True, provinceNameSort=True, province=True, dealerAddress=True, shopBusinessType=True, dealerCode=True)
