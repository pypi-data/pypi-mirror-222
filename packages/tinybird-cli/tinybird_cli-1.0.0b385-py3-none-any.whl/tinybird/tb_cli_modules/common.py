...
variable to '1' or 'true'.""")
        ...
        if int(pyv[0]) == 3 and int(pyv[1]) <= 7:
            formatter.write_paragraph()
            formatter.write_heading('Deprecation warning')
            formatter.write_text(f"""
  You are using CLI under Python v{'.'.join(pyv)}.
Starting 2023-08-01, the Tinybird CLI won't support Python versions <= 3.7.
Please, upgrade to a supported runtime, or pin a compatible CLI
version in your requirements.""")
