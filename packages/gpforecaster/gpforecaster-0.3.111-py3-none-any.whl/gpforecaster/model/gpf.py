...
class GPF:
    ...
    def __init__(
        ...
        self.groups, self.scaler, self.scaler_x_values = self._preprocess(
    ...
    def _preprocess(
        ...
        def convert_and_transform_data(
            ...
            return torch.from_numpy(data_np).to(torch.float32).to(device)
        ...
        scaler_x_values = StandardScaler()
        if self.scale_x_values:
            groups["train"]["x_values"] = convert_and_transform_data(
                groups["train"]["x_values"], scaler_x_values, fit=True
            )
            groups["predict"]["x_values"] = convert_and_transform_data(
                groups["predict"]["x_values"], scaler_x_values, fit=False
            )
        else:
            groups["train"]["x_values"] = (
                torch.from_numpy(groups["train"]["x_values"])
                .to(torch.float32)
                .to(device)
            )
            groups["predict"]["x_values"] = (
                torch.from_numpy(groups["predict"]["x_values"])
                .to(torch.float32)
                .to(device)
            )
        ...
        return groups, scaler, scaler_x_values
