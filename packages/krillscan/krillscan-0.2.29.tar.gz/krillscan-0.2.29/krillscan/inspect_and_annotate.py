# -*- coding: utf-8 -*-
"""
Created on Tue Jul 18 09:26:18 2023
...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul 12 12:55:08 2023
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        top3_layout.addWidget(QtWidgets.QLabel('Annotation:'))
    ...
    def plot_annotation_box(self,annotation_row):
        ...
        self.ax[1].plot(line_x,line_y,'-k',linewidth=.75)
        self.ax[1].text(xmin,ymax,c_label,size=8)
    ...
    def box_select_callback(self,eclick, erelease):
        ...
        a=pd.DataFrame({'t1': pd.Series(t1,dtype='float'),
               't2': pd.Series(t2,dtype='float'),
               'd1': pd.Series(f1,dtype='float'),
               'd2': pd.Series(f2,dtype='float'),
    ...
    def plot_echogram(self):    
        ...
        # breakpoint()
        self.statusBar().showMessage(self.filenames[self.filecounter],2147483640)

        if self.choose_freq.currentText() == 'RGB':
            
            flen = self.xr_sv.shape[0]
            if flen>3:
                flen=3
             
            sv_rgb=np.zeros([self.xr_sv.shape[1],self.xr_sv.shape[2],3])    
            for ix_f in range(flen):    
                sv_rgb[:,:,ix_f] = np.squeeze( self.xr_sv[ix_f,:,:])
                
            dbmin=-90
            dbmax=-30
            sv_rgb[sv_rgb>dbmax]=dbmax
            sv_rgb[sv_rgb<dbmin]=dbmin
            
            sv_rgb = (sv_rgb  - dbmin ) /(dbmax -dbmin)
            ...
            im = ax[1].imshow( sv_rgb,extent=ext,aspect='auto')
    ...
    # plot annotations
        if self.annotation.shape[0]>0:
              x_lims = mdates.date2num( [  self.xr_sv.coords['time'].min().values , self.xr_sv.coords['time'].max().values ])
              t1_m = self.annotation['t1'] 
              t2_m = self.annotation['t2'] 
              ix=(t1_m > x_lims[0]  ) &  (t1_m < x_lims[1]  )  
              if np.sum(ix)>0:
                  ix=np.where(ix)[0]
                  print('ix is')
                  print(ix)
        ...
        if self.butt_addarea.isEnabled() & self.butt_removearea.isEnabled():     
            self.toggle_selector=RectangleSelector(ax[1], self.box_select_callback,
                                                drawtype='box', useblit=False,
                                                button=[1],  # disable middle button                                  
                                                interactive=False,rectprops=dict(facecolor="k", edgecolor="black", alpha=0.1, fill=True))
                    

        self.canvas.fig.tight_layout()
        self.canvas.draw()
        
        self.cid_annot=self.canvas.fig.canvas.mpl_connect('button_press_event', self.onclick)
    ...
    def read_file_and_mask(self):    
        if self.filecounter>=0:        
            ...
            if os.path.isfile( fname[:-12] + '_annotations.csv'):     
                self.annotation=  pd.read_csv( fname[:-12] + '_annotations.csv',index_col=0) 
            else:             
                self.annotation= pd.DataFrame({'t1': pd.Series(dtype='float'),
                   't2': pd.Series(dtype='float'),
                   'd1': pd.Series(dtype='float'),
                   'd2': pd.Series(dtype='float'),
    ...
    def mask_removearea(self):
        ...
        # if returnValue == QtWidgets.QMessageBox.Ok:
            ...
            self.canvas.fig.canvas.mpl_disconnect(self.cid_annot)
    ...
    def mask_addarea(self):
        ...
        # if returnValue == QtWidgets.QMessageBox.Ok:
            ...
            self.cid2=self.canvas.fig.canvas.mpl_connect('button_press_event', self.onclick_draw)
            self.line_2 =self.ax[1].plot(self.draw_x,self.draw_y,'.-k')        
            # self.plot_echogram()   
            self.drawexitm = QtWidgets.QShortcut(QtCore.Qt.Key_Return, self)
    ...
    def save_changes(self):
        ...
        self.annotation.to_csv( fname[:-12] + '_annotations.csv' )
