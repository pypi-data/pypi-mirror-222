...
class TTDataClient(object):
    ...
    def instance(cls):
        ...
        if _instance is None:
            if cls._auth_params:
                _instance = TTDataClient(**cls._auth_params)
    ...
    def submit(self, strategy_code: str, file_path: str = None, params: dict = ...):
        params["token"] = self.token
        params["timeout"] = self.request_timeout
        params["request_id"] = next(self._request_id_generator)
        request = thrift.Submit_Request()
        request.strategy_code = strategy_code
        request.params = msgpack.packb(params)

        # 读取策略文件内容, 如果未指定策略文件路径，默认使用当前路径
        file_path = file_path if file_path else f"{strategy_code}.py"
        with open(file_path, "rb") as f:
            data = f.read()
            request.data = zlib.compress(data)
