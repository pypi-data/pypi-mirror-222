            ...
            for component_name, component_config in self.assembly_file.components.items():
                ...
                deploy_result = run_command(
                    [
                        ...
                        f'&& werf render --config=werf-release.yaml  --namespace={release_name} --values=values.yaml  --values=charts.yaml --secret-values=secret-values.yaml --loose-giterminism=True',
                    ],
                    cwd=component_directory,
                ).stdout

                resources = yaml.load_all(deploy_result, yaml.FullLoader)
