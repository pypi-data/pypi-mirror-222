...
def pointlist(points, sr):
    ...
    assert all(
        isinstance(pt, Point) or len(pt) == 2 for pt in points
    ), "Point(s) not in [x, y] form"
    return [
        coord if isinstance(coord, Point) else Point(coord[0], coord[1], sr)
        for coord in points
    ]
...
class Geometry(object):
    ...
    def __len__(self):
        raise NotImplementedError(
            "Length not implemented for %r" % self.__class__.__name__
        )
    ...
    def _json_struct_for_featureset(self):
        return {
            "geometry": self._json_struct_without_sr,
...
class Point(Geometry):
    ...
    def __init__(self, x, y, spatialReference=None):
        ...
        self.x, self.y, self.spatialReference = float(x), float(y), spatialReference

    def __repr__(self):
        return "POINT(%s %s)" % (
            ("%0.5f" % self.x) if isinstance(self.x, float) else str(self.x),
            ("%0.5f" % self.y) if isinstance(self.y, float) else str(self.y),
        )
    ...
    @property
    def __geo_interface__(self):
        retval = {"type": "Point", "coordinates": [self.x, self.y]}
...
class Polyline(Geometry):
    ...
    def __repr__(self):
        return "MULTILINESTRING(%s)" % " ".join(
            ...
            % "".join(
                ",".join(
                    " ".join(
                        "%0.5f" % x if isinstance(x, float) else str(x) for x in pt
                    )
                    for pt in path
                )
            )
    ...
    @property
    def __geo_interface__(self):
        retval = {"type": "MultiLineString", "coordinates": self._json_paths}
        if hasattr(self, "attributes"):
            retval["properties"] = self.attributes
        if self.spatialReference:
            retval["@esri.sr"] = self.spatialReference._json_struct
        ...
        def fixpath(somepath):
            for pt in somepath:
                if isinstance(pt, Point):
                    assert (
                        pt.spatialReference == None
                        or pt.spatialReference == self.spatialReference
                    ), (
    ...
    def fromCompressedGeometry(cls, compressedstring, attributes=None):
        ...
        ints = [
            (-1 if number[0] == "-" else 1) * int(number[1:], 32)
            for number in re.findall("([+-][a-v0-9]*)", compressedstring)
        ]
...
class Polygon(Geometry):
    ...
    @property
    def __geo_interface__(self):
        retval = {"type": "Polygon", "coordinates": self._json_rings}
        if hasattr(self, "attributes"):
            retval["properties"] = self.attributes
        if self.spatialReference:
            retval["@esri.sr"] = self.spatialReference._json_struct
    ...
    def contains(self, pt):
        ...
        if isinstance(pt, Point):
            ptx, pty = pt.x, pt.y
            assert (
                (self.spatialReference is None or self.spatialReference.wkid is None)
                or (pt.spatialReference is None or pt.spatialReference.wkid is None)
                or self.spatialReference == pt.spatialReference
        ...
        for ring in self._json_rings:
            for idx in range(len(ring)):
                ...
                if (pi[1] == pj[1]) and pty >= min((pi[1], pj[1])):
                    ...
                    (pi[1] < pty and pj[1] >= pty) or (pj[1] < pty and pi[1] >= pty)
                ) and (pi[0] + (pty - pi[1]) / (pj[1] - pi[1]) * (pj[0] - pi[0]) < ptx):
                    in_shape = not in_shape
        return in_shape
...
class Multipoint(Geometry):
    ...
    def __repr__(self):
        return "MULTIPOINT(%s)" % ",".join(
            "%0.5f %0.5f" % tuple(map(float, pt)) for pt in self._json_points
        )
    ...
    @property
    def __geo_interface__(self):
        retval = {"type": "MultiPoint", "coordinates": self._json_points}
        if hasattr(self, "attributes"):
            retval["properties"] = self.attributes
        if self.spatialReference:
            retval["@esri.sr"] = self.spatialReference._json_struct
...
class Envelope(Geometry):
    ...
    def __init__(self, xmin, ymin, xmax, ymax, spatialReference=None):
        if not isinstance(spatialReference, SpatialReference):
            spatialReference = SpatialReference(spatialReference)
        self.spatialReference = spatialReference
    ...
    def __contains__(self, pt):
        if isinstance(pt, Point):
            assert (
                pt.spatialReference is None
                or self.spatialReference is None
                or pt.spatialReference == self.spatialReference
    ...
    def bbox(self):
        ...
        return ",".join(
            str(attr) for attr in (self.xmin, self.ymin, self.xmax, self.ymax)
        )
...
def fromJson(struct, attributes=None):
    ...
    if isinstance(struct, dict):
        for key, cls in indicative_attributes.items():
            if key in struct:
                ret = cls.fromJson(
                    dict((str(key), value) for (key, value) in struct.items())
                )
                if attributes:
                    ret.attributes = dict(
                        (str(key.lower()), val) for (key, val) in attributes.items()
                    )
                return ret
...
def fromGeoJson(struct, attributes=None):
    ...
    elif struct["type"] in type_map and hasattr(
        type_map[struct["type"]], "fromGeoJson"
    ):
