...
class randomaffine_imgs:
    def __init__(self, rotate:list[float], transx:list[float], transy:list[float], scale:list[float]):
        self.rot_deg = 0 if rotate == None else random.uniform(rotate[0], rotate[1])
        self.transx = 0 if transx == None else random.uniform(transx[0], transx[1])
        self.transy = 0 if transy == None else random.uniform(transy[0], transy[1])
        self.scale = 1 if scale == None else random.uniform(min(scale), max(scale))
