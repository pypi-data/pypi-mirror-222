...
class Question:
    ...
    # ASK INTEGER
    def ask_integer(self, question, count=0, n_trials=None): 
        """Integer user input
    ...
    # ASK FLOAT
    def ask_float(self, question, count=0): 
        """Float user input
...
def _save_estimates(star, variables=['star','numax','dnu','snr']):
    ...
    if 'best' in star.params:
        ...
        star.params['numax'], star.params['snr'] = results[1], results[3]
    return star
...
def setup_dirs(args, note='', dl_dict={}):
    ...
    if dl_dict:
        ...
        for infile, outfile in dl_dict.items():
            command = 'curl %s > %s' % (infile, outfile)
