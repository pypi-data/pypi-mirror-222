...
def select_trial(star):
    ...
    plt.tight_layout()
    plt.show()
    trial = Question().ask_integer('Which estimate would you like to use? ', n_trials=star.params['n_trials'])
    if trial is not None:
        if trial == 0:
            numax = Question().ask_float('numax=')
            if numax is not None:
                star.params['numax'] = numax
                print('Confirming numax of %.2f muHz as initial guess' % numax)
        else:
            star.params['best'] = trial
            print('Selecting model %d' % trial)
    plt.close()
    return star
