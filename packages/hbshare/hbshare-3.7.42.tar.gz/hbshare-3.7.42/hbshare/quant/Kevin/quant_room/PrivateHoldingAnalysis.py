...
class HoldingCompare:
    ...
    def run(self):
        ...
        bm_distribution = w_df.groupby('benchmark_id').sum().reset_index().replace(
            {"000300": "沪深300", "000905": "中证500", "000852": "中证1000",
             "399303": "国证2000", "other": "超小票"}).set_index('benchmark_id')
        bm_distribution.loc['非权益仓位'] = 100. - bm_distribution.sum()
        # 风格及行业分布
...
if __name__ == '__main__':
    tmp = HoldingAnalysor(
        "宽德九臻", "20230331", "000905").get_construct_result(isPlot=False)
    ...
    # name_list = ['因诺聚配中证500指数增强', '星阔广厦1号中证500指数增强', '顽岩中证500指数增强5号', '半鞅中证500指数增强1号',
    #              '乾象中证500指数增强1号', '概率500指增2号', '千衍六和3号', "信弘征程2号", '铭量中证500增强1号']
    # HoldingCompare('20230331', '000905', name_list).run()
    # concept_compare("20230331", name_list)
    # small_cap_stocks_counts("20230331", name_list)

    # exposure_list = []
    # distribution_list = []
    # for date in ['20230228', '20230412', '20230531', '20230630']:
    #     print(date)
    #     c = HoldingAnalysor("半鞅量化精选", date, "000905").get_construct_result(isPlot=False)
    #     exposure_list.append(c['style_df']['主动暴露'].to_frame(date))
    #     distribution_list.append(c['bm_distribution'].to_frame(date))
    #
    # exposure_df = pd.concat(exposure_list, axis=1)
    # distribution_df = pd.concat(distribution_list, axis=1)
