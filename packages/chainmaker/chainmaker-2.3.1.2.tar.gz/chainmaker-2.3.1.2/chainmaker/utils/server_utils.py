...
class ChainMakerNode:
    ...
    def get_enable_fast_sync(self) -> bool:
        return self.chainmaker_config.get('fast_sync', {}).get('enabled')
    ...
    def get_rpc_tls_mode(self) -> RpcTlsMode:
        rpc_tls_mode = self.chainmaker_config.get('tls', {}).get('mode')
        return RpcTlsMode[rpc_tls_mode]

    def set_rpc_tls_mode(self, rpc_tls_mode: Union[RpcTlsMode, str]):
        if isinstance(rpc_tls_mode, RpcTlsMode):
            rpc_tls_mode = rpc_tls_mode.name
        self.modify_chainmaker_yml(net={'tls': {'mode': rpc_tls_mode}})
        return self
...
class ChainMakerCluster:
    ...
    @classmethod
    def from_chainmaker_go(cls, host: Host, chainmaker_go_path: str = None, crypto_config_path: str = None,
        ...
        # crypto_config = CryptoConfig(crypto_config_path, host.host, rpc_port)
        ...
        for index, release_path in enumerate(release_paths):
            ...
            node = ChainMakerNode(index=index, host=host, release_path=release_path, rpc_port=rpc_port,
                                  crypto_config=crypto_config, cc=cc)
            nodes.append(node)
        return cls(nodes=nodes, crypto_config=None, cc=None)
    ...
    def from_conf(cls, server_config: dict, cc: ChainClientWithEndorsers = None):
        ...
        crypto_config = load_crypto_config(crypto_config_path, host=host_config.get('host', '127.0.0.1'),
                                           rpc_start_port=rpc_start_port)
        cc = cc or crypto_config.new_chain_client(endorsers_cnt=crypto_config.node_cnt, conn_node=0)
