...
class _Pk(_WithPubkeyFile):
    ...
    def to_client_user(self):
        return ClientUser.from_conf(**self.create_client_user_config())
...
class _CryptoConfigBase:
    ...
    def _create_node_config(self, enable_tls: bool = False, conn_node: int = None, node_cnt: int = 4) -> List[dict]:
...
def load_crypto_config(crypto_config_path: Union[Path, str],
                       ...
                       hash_type='sha256') -> CryptoConfig:
    auth_type = guess_auth_type(crypto_config_path)
    if auth_type == AuthType.Public:
        return PkCryptoConfig(crypto_config_path, host, rpc_start_port, hash_type, AuthType.Public)
