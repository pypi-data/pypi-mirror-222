...
' | ''' + (f'sh prepare_pwk.sh {node_cnt} {chain_cnt} {p2p_port} {rpc_port} '
    ...
    def deploy_cert_4_nodes_tbft_enable_vm_go(self, branch=None) -> bool:
        ...
        self.host.run(
            'sh prepare.sh 4 1 11301 12301 32351 22351 23351 -c 1 -l INFO -v true --vtp=tcp --vlog=INFO -j false -jlog=INFO',
            workspace=self.scripts_path)
    ...
    def deploy_cert_10_nodes_tbft_enable_vm_go(self, branch=None, container_name_prefix: str = None) -> bool:
        warnings.warn('即将废弃', DeprecationWarning)
        if branch:
            self.host.run(f'git reset --hard && git remote prune origin && git checkout {branch} && git pull',
                          workspace=self.chainmaker_go_path)
        ...
        self.host.run(
            'sh prepare.sh 10 1 31301 32301 62351 62451 62551 -c 1 -l INFO -v true --vlog=INFO -j false -jlog=INFO',
            workspace=self.scripts_path)
