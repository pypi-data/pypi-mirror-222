...
class iFDO:
    ...
    def set_header_equipment_uri_to_equipment_git_url(self,ifdo_field:str, override:bool = False, ignore_offline:bool = False):
        ...
        image_field = self.findTmpField2(ifdo_field)
        if image_field == "" or 'name' not in image_field or image_field['name'] == "":
            self.prov.log("Error! Can't set "  + ifdo_field + " uri, name not filled.")
            return
    ...
    def set_header_image_project_uri_to_osis_expedition_url(self,override:bool = False):
        ...
        image_project = self.findTmpField2('image-project')
        if image_project == "" or 'name' not in image_project or image_project['name'] == "":
            self.prov.log("Error! Can't set image-project uri, image-project not filled.")
            return
        ...
        if exp_name not in exp_ids:
            self.prov.log("Error! Can't get osis expedition id from image-project " + str(image_project))
            return
    ...
    def set_header_image_event_uri_to_osis_event_url(self,override:bool = False):
        ...
        image_project = self.findTmpField2('image-project')
        if image_project == "" or 'name' not in image_project or image_project['name'] == "":
            self.prov.log("Error! Can't set image-event uri, image-project not filled.")
            return

        # get expedition id
        ## try parse expedition id from project osis uri
        if 'uri' in image_project:
        ...
        # try parse event id
        event_name = self.findTmpField2('image-event:name')
        if event_name == "":
            self.prov.log("Error! Can't set image-event uri, image-event:name not filled.")
            return

        osis_url = self.get_osis_event_url(expedition_id, event_name)
        if osis_url == "":
            self.prov.log("Error! Can't set image-event uri from expedition_id and event_name: " + str(expedition_id) + ", " + event_name)
            return
