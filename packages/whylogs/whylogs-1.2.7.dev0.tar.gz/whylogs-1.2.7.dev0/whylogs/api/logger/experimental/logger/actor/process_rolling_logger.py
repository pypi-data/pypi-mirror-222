...
from whylogs.api.logger.experimental.logger.actor.process_actor import ProcessActor
...
class WhyLabsWriterFactory(WriterFactory):
    def create_writers(self, dataset_id: str) -> List[Writer]:
        return [
            Writers.get(
                "whylabs",
                # TODO get these from the session after that pr is merged
...
class ProcessRollingLogger(ProcessActor[MessageType], DataLogger[Dict[str, ProcessLoggerStatus]]):
    ...
    def log(
        ...
        if dataset_id is None:
            env_id = os.environ.get("WHYLABS_DEFAULT_DATASET_ID")
            # TODO replace this with the new session config after rebasing
            if env_id is None:
                raise Exception("Need to specify a dataset id or set WHYLABS_DEFAULT_DATASET_ID")
            dataset_id = env_id
