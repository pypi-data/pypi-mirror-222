...
class QueueWrapper(Generic[QueueMessageType]):
    @abstractmethod
    def send(self, message: QueueMessageType, timeout: float = 0.1) -> None:
        raise NotImplementedError()

    @abstractmethod
    def send_many(self, message: QueueMessageType, timeout: float = 0.1) -> None:
        raise NotImplementedError()

    @abstractmethod
    def get(self, timeout: float = 0.1) -> Optional[QueueMessageType]:
        raise NotImplementedError()

    @abstractmethod
    def get_many(self, timeout: float = 0.1, max: Optional[int] = None) -> List[QueueMessageType]:
        raise NotImplementedError()
