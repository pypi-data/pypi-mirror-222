...
class UserRelateToUserService():
    ...
    def distribution_relate(*args, params: dict = None, **kwargs):
        ...
        relate_user_obj = UserRelateToUser.objects.extra(
            ...
            with_full_name=F("with_user__full_name"),
            with_nickname=F("with_user__nickname")
        ).filter(user_relate_type__relate_key=relate_key)

        # 分級別查詢
        if select_level == "first":
            relate_user_obj = relate_user_obj.filter(with_user_id=user_id)
            ...
            relate_user_obj = relate_user_obj.filter(with_user_id__in=second_users + first_users + [user_id])
        ...
        for j in page_list:
            ...
            # 判断用户分销等级
            j["level"] = 3
            if select_level == "all" and j["user_id"] in first_users:
                j["level"] = 1
            elif select_level == "all" and j["user_id"] in second_users:
                j["level"] = 2
        # ------------------------- section 獲取用戶信息 end   ------------------------------------
        ...
        data_map = {}
        EnrollStatisticsServices, err = dynamic_load_class(
            import_path="xj_enroll.service.enroll_statistics_services",
        ...
        #  判断该用户是否是业务人员
        role_info, err = RoleService.get_role_list(params={"user_id": user_id}, only_first=True)
        is_salesperson = 0
