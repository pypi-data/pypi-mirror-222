...
class ACBulkBreakBulkCargo:
    def __init__(self, type_of_cargo: float, total_metric_or_revenue_ton: float):
        self.type_of_cargo = type_of_cargo
        self.total_metric_or_revenue_ton = total_metric_or_revenue_ton
    ...
    def calculate_bulk_break_bulk_cargo(self, arrastre_charge_rate: float):
        total_arrastre_charge = round(
            self.total_metric_or_revenue_ton * arrastre_charge_rate, 2
        )
        return total_arrastre_charge


class ACBulkBreakBulkCargoImport(ACBulkBreakBulkCargo):
    def __init__(self, type_of_cargo: float, total_metric_or_revenue_ton: float) -> None:
        super().__init__(type_of_cargo, total_metric_or_revenue_ton)
    ...
    def calculate_bulk_break_bulk_cargo_import(self):
        if self.type_of_cargo == self.general_cargo:
            self.arrastre_charge_rate = self.general_cargo_rate

        elif self.type_of_cargo == self.bagged_cargo:
            self.arrastre_charge_rate = self.bagged_cargo_rate

        elif self.type_of_cargo == self.palletized:
            self.arrastre_charge_rate = self.palletized_rate

        elif self.type_of_cargo == self.unpacked_fish:
            self.arrastre_charge_rate = self.unpacked_fish_rate

        elif self.type_of_cargo == self.fish_in_cartons:
            self.arrastre_charge_rate = self.fish_in_cartons_rate

        elif self.type_of_cargo == self.steel_products:
            self.arrastre_charge_rate = self.steel_products_rate

        elif self.type_of_cargo == self.logs:
            self.arrastre_charge_rate = self.logs_rate

        elif self.type_of_cargo == self.lumber:
            self.arrastre_charge_rate = self.lumber_rate

        elif self.type_of_cargo == self.heavy_lift_5to15_tons:
            self.arrastre_charge_rate = self.heavy_lift_5to15_tons_rate

        elif self.type_of_cargo == self.heavy_lift_over15to20_tons:
            self.arrastre_charge_rate = self.heavy_lift_over15to20_tons_rate

        elif self.type_of_cargo == self.heavy_lift_over20_tons:
            self.arrastre_charge_rate = self.heavy_lift_over20_tons_rate

        elif self.type_of_cargo == self.checking_charge_shipside:
            self.arrastre_charge_rate = self.checking_charge_shipside_rate

        self.calculate_bulk_break_bulk_cargo(self.arrastre_charge_rate)
...
class ACBulkBreakBulkCargoExport(ACBulkBreakBulkCargo):
    ...
    def calculate_bulk_break_bulk_cargo_export(self):
        if self.type_of_cargo == self.general_cargo:
            self.arrastre_charge_rate = self.general_cargo_rate
