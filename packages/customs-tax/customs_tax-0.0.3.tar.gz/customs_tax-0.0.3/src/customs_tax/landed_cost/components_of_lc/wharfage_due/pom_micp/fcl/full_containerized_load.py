class WDContainerizedCargo:
    ...
    def calculate_fcl(self, wharfage_due_rate) -> float:
        total_wharfage_due = round(
            self.total_number_of_containers * wharfage_due_rate, 2
        )
...
class WDContainerizedCargoImport(WDContainerizedCargo):
    ...
    def calculate_fcl_import(self):
        if self.size_of_container == self.twenty_footer:
            self.wharfage_due_rate = self.twenty_footer_rate

        elif self.size_of_container == self.forty_footer:
            self.wharfage_due_rate = self.forty_footer_rate

        elif self.size_of_container == self.forty_five_footer:
            self.wharfage_due_rate = self.forty_five_footer_rate

        return self.calculate_fcl(self.wharfage_due_rate)
