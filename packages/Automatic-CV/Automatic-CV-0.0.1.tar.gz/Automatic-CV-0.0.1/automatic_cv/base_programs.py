...
class OCV( BiologicProgram ):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.field_titles = [ 'Time', 'Voltage' ]
...
class JV_Scan( BiologicProgram ):
    ...
    """
    """
    Danger's note: I suspend that there is a typo here and it should be CV_scan.
    """
    def __init__(
        self,
        device,
        ...
        **kwargs
        ...
        self.field_titles = [ 'Voltage [V]', 'Current [A]', 'Time [s]', 'Power [W]' ]
        
        self._fields = namedtuple( 'CV_Datum', [
           'voltage', 'current', 'Time', 'power'
        ] )
...
class MPP( MPP_Tracking ):
    ...
    def run( self, data = 'data', by_channel = False, cv_params = {} ):
        """
...
class MPP_CV( BiologicProgram ):
    ...
    def run( self, data = 'data', by_channel = False, cv_params = {} ):
        ...
        ocv_loc = 'voc' if by_channel else 'voc.csv'
        cv_loc  = 'cv'  if by_channel else 'cv.csv'
        mpp_loc = 'mpp' if by_channel else 'mpp.csv'

        mpp_loc = os.path.join( data, mpp_loc )
        ocv_loc = os.path.join( data, ocv_loc )
        cv_loc  = os.path.join( data, cv_loc )
        ...
        #--- init ---
        self.voc = self._run_ocv( ocv_loc, by_channel = by_channel ) # voc
        self.v_mpp = self._run_cv( self.voc, cv_loc, by_channel = by_channel, cv_params = cv_params ) # jv
    ...
    def _run_cv( self, voc, file, by_channel = False, cv_params = {} ):
        """
        Runs cV_Scan program to obtain initial v_mpp.
