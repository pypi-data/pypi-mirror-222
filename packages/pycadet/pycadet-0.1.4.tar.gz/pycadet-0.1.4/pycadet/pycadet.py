...
def make_3D_cavity(cavity):
    ...
    shape = cavity.shape[0]
    x0, y0 = shape // 2 - 0.5, shape // 2 - 0.5

    # DE-ROTATES CAVITY
    cen = center_of_mass(cavity)
    phi = np.arctan2(cen[0]-x0, cen[1]-y0)
    ...
    for n in range(shape):
        rang = np.where(cavity[:,n] > 0, np.arange(0,shape), 0)
    ...
    cube = np.zeros((shape,shape,shape))
    for m, w, i in zip(means, widths, indices):
        x, y = np.indices((shape, shape))
