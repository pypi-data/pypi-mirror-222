...
def surface(self, 
    ...
    # Get records
    records_df = database.get_records_df(style='calculation_surface_energy_static',
    ...
    for imp_df, pot_id, pot_key, imp_id, imp_key in self.iter_imp_df(records_df):
        ...
        if prop.freesurfaces.exists and runall is False:
            print('skipped')
            num_skipped += 1
            continue
        ...
        prop.freesurfaces.data = transform_imp_df(imp_df)
        ...
        for composition in np.unique(imp_df.composition):
            ...
            # Build and save csv
            surface_csv(comp_df, outputpath, pot_id, imp_id, composition)

        # Build model component
        prop.freesurfaces.exists = True
        model = prop.model['per-potential-properties']
        prop.freesurfaces.build_model(model)
...
def transform_imp_df(imp_df):
    ...
    for i in imp_df.index:
        ...
        # Extract plane and slice position from surface_id
        surface = series.surface_id.replace(series.prototype+'--', '')
        if '-' in surface:
            plane, pslice = surface.split('-')
        else:
            plane = surface
            pslice = ''
        ...
        dat['a'] = series.a
        dat['surface'] = f'({plane}) {pslice}'.strip()
        dat['E_f'] = uc.get_in_units(series.E_surface_f, 'mJ/m^2')
        new_df.append(dat)
    new_df = pd.DataFrame(new_df)

    return new_df
        
def surface_csv(df: pd.DataFrame,
                outputpath: Path,
                potential: str,
                implementation: str,
                composition: str):
    ...
    for i in df.index:
        ...
        dat['a'] = series.a
        dat['surface_id'] = series.surface_id
        dat['surface'] = series.surface_id.replace(series.prototype+'--', '')
        dat['gamma_fs'] = '%.2f' % uc.get_in_units(series.E_surface_f, 'mJ/m^2')
        csv_df.append(dat)

    sort_keys = ['prototype', 'a', 'gamma_fs']
    csv_df = pd.DataFrame(csv_df).sort_values(sort_keys)

    filepath = Path(outputpath, potential, implementation,
                    f'freesurface.{composition}.csv')
