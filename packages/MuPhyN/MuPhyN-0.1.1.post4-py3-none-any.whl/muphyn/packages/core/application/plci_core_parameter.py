...
class Parameter:
    ...
    def fromDict(dict_: Dict) -> "Parameter":
        ...
        name = "" if "name" not in dict_ else dict_["name"]
        type_ = None if "type" not in dict_ else get_data_type(dict_["type"])
        value = None if "value" not in dict_ else dict_["value"]
