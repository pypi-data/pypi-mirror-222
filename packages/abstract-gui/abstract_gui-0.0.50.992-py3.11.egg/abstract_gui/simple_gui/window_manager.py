...
class WindowManager:
    ...
    def t_or_f_obj_eq(self, event=None, obj=None):
        return event == obj
    def verify_window(self, window: any = None) -> bool:
        """
        Verifies if the given object is a valid PySimpleGUI window.
        ...
        """
        verify = self.search_global_windows(window=window)
        if verify != False:
            return True
        return False
    ...
    def get_event(self, window=None):
        ...
        if name is not False:
            return self.all_windows[self.search_global_windows(window)]['last_event']
