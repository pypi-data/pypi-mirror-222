...
class DoHomeGateway:
    ...
    async def discover_ip(self, sid: str) -> str:
        ...
        responses = await create_task(self._broadcast.send_request(
    ...
    async def discover_devices(self, sends_count=3) -> List[str]:
        ...
        while sends_count > 0:
            await create_task(
                self._broadcast.send_request(REQUEST_PING, receive=False)
            )
            sends_count -= 1
        responses = await create_task(self._broadcast.receive())
