...
class DoHomeLight():
    ...
    async def get_state(self) -> dict:
        ...
        raw_state = await create_task(self.get_raw_state())
    ...
    async def get_raw_state(self):
        ...
        return await create_task(self._send_request(
            format_request(self._sids, CMD_GET_STATE)
        ))

    async def get_time(self):
        """Reads time from the device"""
        await create_task(self._send_request(
    ...
    async def _send_request(self, request: str):
        response_data = await create_task(self._transport.send_request(request))
