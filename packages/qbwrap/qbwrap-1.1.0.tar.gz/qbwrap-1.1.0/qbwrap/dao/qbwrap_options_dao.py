...
from ..util.reference_util import is_reference_string
from ..util.reference_util import reference_string_to_path
from ..util.size_util import size_bytes
...
class QBwrapOptionsDAO(QBwrapStanzaDelegateDAO):
    ...
    def get_unshare_stanza_options(self) -> List[str]:
        ...
        for field_name in unshare_stanza.get_field_names():
    ...
    def __dereference(self, maybe_reference: str) -> str:
        if is_reference_string(maybe_reference):
            reference = self.__reference_dao.get_reference(
                reference_string_to_path(maybe_reference)
            )
