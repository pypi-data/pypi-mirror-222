...
class PushPlan(models.Model):
    ...
    role_id = models.IntegerField(verbose_name='权限id', primary_key=False, help_text='')
    group_id = models.IntegerField(verbose_name='分组id', primary_key=False, help_text='')
    user_id_list = models.JSONField(verbose_name='用户id列表', blank=True, null=True, help_text='')
    node_to_action_id = models.IntegerField(verbose_name='节点对动作ID', primary_key=False, help_text='')
...
class PushTemplate(models.Model):
    id = models.AutoField(verbose_name='ID', primary_key=True)
    value = models.CharField(verbose_name='模板搜索关键词', max_length=128, blank=True, null=True,
                             help_text='')
    template = models.CharField(verbose_name='内容推送模板，变量使用{}包裹', max_length=128, blank=True, null=True,
                               help_text='')
