...
# with open("changyong.txt", mode='r', encoding='utf-8') as f:
#     data = f.readlines()
#     data = map(lambda l: l.split("\t")[-1][:-1].strip(), data)
#     data = ''.join(data)

data = "的、一、是、在、不、了、有、和、人、这、中、大、为、上、个、国、我、以、要、他、" \
...
similar = open("similar.txt", "r")

f_out = open("cy_similar_500.txt", "w")
all_similar = ""
for line in similar.readlines():
    word = line.split(" ")
    main_word = word[0]
    tmp_word = ""
    if main_word in data:
        tmp_word += main_word
        if len(word) > 1:
            tmp_word += " "
            for s in word[1][:-1]:
                if s in data:
                    tmp_word += s
        all_similar += tmp_word
        f_out.write(tmp_word+"\n")

for i in data:
    if i not in all_similar:
        print("not in similar:", i)
        f_out.write(i+"\n")
