...
class FileUploadManager(object):
    ...
    def upload_file_like_thread(self, options: FileLikeUploadOptions, critical=False):
        if self._use_s3_direct_upload(options):
            self._initiate_upload(
                options=options,
                critical=critical,
    ...
    def upload_remote_asset_thread(
        self, options: RemoteAssetsUploadOptions, critical=False
    ):
        self._initiate_upload(options, critical, upload_remote_asset)

    def _initiate_upload(
        self,
        options: Union[
            FileUploadOptions, FileLikeUploadOptions, RemoteAssetsUploadOptions
        ],
        critical: bool,
        uploader,
        s3_multipart=False,
