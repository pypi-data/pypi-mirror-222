...
class RemoteQClient(BaseQClient):
    def __init__(self) -> None:
        ...
        self.api_client = QElectronAPI(qserver_url)
    ...
    def register(self, qscripts, executors, qelectron_info, qnode_specs) -> List[Dict]:
        ...
        print(f"create circuit request body: {request_body}")
        res = self.api_client.post("/circuits", request_options={
            "data": request_body.json(),
        })
        print(f"create circuit response: {res.text}")
        return res.json()
    ...
    def start(self, circuit_ids: List[str]):
        res = self.api_client.post("/qdispatch", request_options={
            "json": circuit_ids
        })
        return res
    ...
    def submit(
        ...
        print("Uploading circuits and diagrams")
        ...
        print(f"Started circuits {circuit_ids}")
    ...
    def get_results(self, batch_id: str):
        ...
        while len(remaining_circuit_ids) > 0:
            ...
            print(f"Waiting for results from batch {batch_id}...")
            time.sleep(5)
