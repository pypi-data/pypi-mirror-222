...
def get_package_and_version() -> Tuple[bool, str, str]:
    """
    @return: A tuple (is installed package, package name, version)
...
def get_bin_directory() -> Path:
    """
    :return: The bin directory, generated after building the tool.
    There are three cases:
    1. A user - doesn't have the bin directory, we return the current working directory
    2. A dev that builds the tool locally - we return ../../bin
    3. CI - we return ../bin
    """
    bin_dir = Path(__file__).parents[2] / "certora_bins"
    if bin_dir.is_dir():
        return bin_dir.resolve()
    bin_dir_in_ci = Path(__file__).parents[1] / "certora_bins"
    if bin_dir_in_ci.is_dir():
        return bin_dir_in_ci.resolve()
    return Path.cwd()


def default_jar_path() -> Path:
    return get_bin_directory() / "emv.jar"
...
class Mode(NoValEnum):
    ...
    6. CLI parameters consist of one object (.o|so) file. The object file already contains `assert` statements.
    """
...
def find_jar(jar_name: str) -> Path:
    ...
    :returns: A `Path` object that represents the absolute path to the jar file. If the jar file is found in the
             `certora_bins` directory (for dev users or CI), the function returns the absolute path to that jar file.
             Otherwise, it returns the absolute path to the jar file located in the `certora_jars` directory
             (for customers).
    """

    jar_path_prefix = Path(__file__).parents[2]
    if jar_path_prefix.name not in ["EVMVerifier", "repo"]:
    ...
    jar_in_bin_path = jar_path_prefix / "certora_bins" / jar_path_suffix
