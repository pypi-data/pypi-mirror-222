...
from hydronaut.types import Number, Path, OptimizationValue
...
class Experiment():
    ...
    * Add methods for logging parameters, artefacts, etc.
    ...
    def __call__(self) -> OptimizationValue:
        '''
        Run the experiment and return the objective value(s). This must also
        save the trained model to self.model.
    ...
    def log_artifact(self, local_path: Path, artifact_path: Optional[Path] = None) -> None:
        '''
    ...
    def log_artifacts(self, local_dir: Path, artifact_path: Optional[Path] = None) -> None:
        '''
    ...
    def log_metrics(self, metrics: dict[str, Number], step: Optional[int] = None) -> None:
        '''
