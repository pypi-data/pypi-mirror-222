...
def render_console_frames(frame_list: FrameStoreList, globals: Globals) -> str:
    ...
    for frame_index, frame_item in enumerate(frame_list):
        ...
        if frame_item.type == "grid" or frame_item.type == "table_header" or frame_item.type == "text":
            if frame_item.frame_styles.multiline is False or frame_item.frame_styles.max_lines == 1:
                return_string += render_console_single_line_grid(grid_frame_dict=frame_item, globals=globals)
            else:
                return_string += render_console_multi_line_grid(grid_frame_dict=frame_item, globals=globals)

        if frame_item.type == "table_body":
            return_string += render_console_table_frame(table_frame_store=frame_item, globals=globals)
