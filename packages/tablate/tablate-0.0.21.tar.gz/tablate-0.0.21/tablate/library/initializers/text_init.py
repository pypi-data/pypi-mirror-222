...
def text_init(text: Union[str, int, float],
              ...
              globals: Globals) -> GridFrameStore:
    ...
    text_column: BaseColumnInput = {
        "string": text,
        "padding": frame_padding if frame_padding else globals.console.column_styles.padding if globals.console.column_styles.padding is not None else column_padding_default,
        "divider": "thin"
    }

    columns = calc_column_widths(columns=[text_column], globals=globals)
    # todo: pesky warning
