...
from tablate.type.primitives import FrameDivider, ColumnDivider, Multiline, MaxLines, HideHeader, ColumnPadding, \
    TextStyle, TextAlign, TextColor, Background, HtmlPxMultiplier, BackgroundPadding
...
def table_init(columns: List[TableColumnInput],
               ...
               options: Options) -> (Union[TableHeaderFrameStore, None], TableBodyFrameStore):
    columns = copy.deepcopy(columns)
    rows = copy.deepcopy(rows)

    background_padding = background_padding if background_padding is not None else set_attr_resolver(
        instance=options.console.outer_styles,
    ...
    table_all_frame_defaults = base_frame_mapper(frame_input=options.console.frame_styles)
    table_all_frame_defaults.multiline = options.console.frame_styles.multiline if options.console.frame_styles.multiline else table_multiline_default
    ...
    table_styles = style_mapper(base_input=BaseStylesInput(frame_styles=FrameStore(frame_divider=frame_divider,
                                ...
                                frame_default=table_all_frame_defaults,
                                column_default=None,
                                text_default=None,
    ...
    header_base_frame_defaults.multiline = multiline_header
    ...
    compiled_header_styles = style_mapper(base_input=header_styles,
                                          ...
                                          frame_default=header_base_frame_defaults,
                                          column_default=table_styles.column_styles,
                                          text_default=header_base_text_defaults,
                                          html_px_multiplier=html_px_multiplier,
                                          options=options)

    compiled_body_styles = style_mapper(base_input=body_styles,
                                        html_input=html_body_styles,
                                        frame_default=table_styles.frame_styles,
                                        column_default=table_styles.column_styles,
                                        text_default=table_styles.text_styles,
                                        html_px_multiplier=html_px_multiplier,
                                        options=options)

    html_body_rows_styles = set_attr_resolver(html_body_styles, "html_row_styles", HtmlTableRowsStore())

    body_base_row_styles = base_rows_mapper(base_rows_input=TableRowsStore(row_line_divider=row_line_divider,
    ...
    if hasattr(body_styles, "row_styles"):
        body_base_row_styles = base_rows_mapper(
            base_rows_input=TableRowsStore(row_line_divider=set_attr_resolver(instance=body_styles.row_styles,
    ...
    columns = calc_column_widths(columns=columns, options=options)
