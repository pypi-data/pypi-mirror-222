...
def process_frame_list(frame_list: dict, globals: Globals) -> FrameStoreList:
    processed_frame_list: FrameStoreList = []
    for frame_key, frame_item in frame_list.items():
        if frame_item["type"] == "text":
            processed_frame_list.append(text_init(**frame_item["options"], globals=globals))
        if frame_item["type"] == "grid":
            processed_frame_list.append(grid_init(**frame_item["options"], globals=globals))
        if frame_item["type"] == "table":
            table_header, table_body = table_init(**frame_item["options"], globals=globals)
            if table_header:
