...
def list_frames(frame_list: dict):
    print()
    ...
    frame_globals = ascii_text_styler(string="Options", column_dict={"text_style": "bold"}) + ascii_terminator()
    header_string += f"{divider}{padding}{frame_globals}{padding}"
    print(header_string)
    ...
    for frame_key, frame_item in frame_list.items():
        ...
        item_globals_list = []
        item_globals_list.append("hide_header") if "hide_header" in frame_item["options"] and frame_item["options"]["hide_header"] is True else None
        item_globals_list.append("multiline") if frame_item["options"]["multiline"] is True else None
        item_globals_list.append(
            f"max_lines: {frame_item['options']['max_lines']}") if frame_item["options"]["max_lines"] is True else None
        item_string += f"{divider}{padding}{'; '.join(item_globals_list)}{padding}"
        print(item_string)
        frame_index += 1
    print()