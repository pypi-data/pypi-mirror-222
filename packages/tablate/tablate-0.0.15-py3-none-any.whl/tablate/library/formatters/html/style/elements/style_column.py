...
def style_column(column_store: HtmlColumnBase,
    ...
    if column_padding is not None:
        if type(column_padding) == int or type(column_padding) == str:
            column_padding_css = f"0 {column_padding * html_px_multiplier}px"
        if type(column_padding) == list:
            if len(column_padding) == 1:
                column_padding_css = f"{column_padding[0] * html_px_multiplier}px"
            if len(column_padding) == 2:
                column_padding_css = f"{column_padding[0] * html_px_multiplier}px {column_padding[1] * html_px_multiplier}px"
            if len(column_padding) == 4:
                column_padding_css = f"{column_padding[0] * html_px_multiplier}px {column_padding[1] * html_px_multiplier}px {column_padding[2] * html_px_multiplier}px {column_padding[3] * html_px_multiplier}px"
        column_styler.add_style_attribute("padding", column_padding_css)
    if column_divider_style is not None or column_divider_weight is not None:
        print(column_divider_style, column_divider_weight)
