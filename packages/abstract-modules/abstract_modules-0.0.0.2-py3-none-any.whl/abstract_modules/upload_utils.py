...
def get_distributions(setup_js):
    ...
    current_version = setup_js['version']
    ...
    while version_numbers[0] != current_version:
        if current_version in version_numbers:
            text = f"Version number {current_version} already exists. The highest version number is {version_numbers[0]}; the original is {setup_js['version']}. Please change it."
        else:
            text = f"Your version number {current_version} is lower than the highest version number {version_numbers[0]}. The original version is {setup_js['version']}. Please change it."

        layout = [
            [get_gui_fun("T", {"text": text})],
            [get_gui_fun('Input', {"default_text": version_numbers[0], "key": "version_number"})],
            [sg.Button("OK")]
        ]
        ...
        override_prompt = f"Would you like to override the version number with {current_version}?"
