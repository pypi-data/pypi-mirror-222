...
def test(args, model, device, datapath, outpath, batchsize=100000):
    ...
    with torch.no_grad():
        ...
        with progress(imageshapelist, desc='Image', total=0 if len(imageshapelist) == 1 else None, ) as pbr:
            ...
            for shape in pbr:
                ...
                tifffile.imwrite(os.path.join(outpath, f"PixelClusterLabels_{image_name}.tif"), TESTPATCHPRED.astype(np.uint16))
                for i in range(len(col_list)):
                    ...
                    VolImage1[mask] = col_list[i, :]
                tifffile.imwrite(os.path.join(outpath, f"PixelClusters_{image_name}.tif"), VolImage1.astype(np.uint8))
                tab = vaex.from_pandas(pd.DataFrame(np.hstack([np.expand_dims(TESTPATCHPRED.reshape(-1), 1),
                                                               dataraw[count:count + (imageshapelist[r][0] * imageshapelist[r][1]), 0:len(hf["data_normalized"].attrs['selmarkernames'])]])))
    ...
    tabledata, my_data_scaled, DistMatrix, uniqueClusters = prep_for_mst(clustertable=my_data, minclustersize=1000,
                                                                         clustersizes=my_data["# Pixels"],
                                                                         outfolder=outpath, includedmarkers=markers)
    mst = generate_mst(distancematrix=DistMatrix, normalizeddf=my_data_scaled[my_data_scaled.columns], colors=col_list, randomseed=0,
                       outfolder=outpath, clusterheatmap=True, displaymarkers=markers,
...
def run_rapid_analysis(args, mst=True, spatial=True):
    ...
    *Spatial*, which helps us to understand the biology behind spatial phenotypic organisation of tissues by
    ...
    tabledata, my_data_scaled, DistMatrix, uniqueClusters = prep_for_mst(clustertable=my_data, minclustersize=10000,
                                                                         clustersizes=my_data["# Pixels"],
