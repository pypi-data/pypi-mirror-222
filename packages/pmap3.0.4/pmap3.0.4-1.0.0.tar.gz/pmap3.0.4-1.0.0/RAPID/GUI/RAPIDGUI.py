...
class RAPIDGUI():
    ...
    def merge_clusters(self, checked=[], addcolorimg=None, addgreyimg=None):
        ...
        if self.analysismode == "Pixel":
            ...
            for i in range(len(labelimg)):
                self.labeledimgs[imganalysisnum+i] = labelimg[i, :self.imageshapelist[i][0], :self.imageshapelist[i][1]].astype(self.labeledimgs[imganalysisnum+i].dtype) - 1
    ...
    def minimum_spanning_tree(self, clusteringindex=None):
        ...
        self.set_invisible(self.viewer)
        self.viewer.add_image(arrays, name=f"MST (Pixel {clustermodeindex+1}" if ispixelcluster else f"MST (Object {clustermodeindex+1})", blending="additive")
