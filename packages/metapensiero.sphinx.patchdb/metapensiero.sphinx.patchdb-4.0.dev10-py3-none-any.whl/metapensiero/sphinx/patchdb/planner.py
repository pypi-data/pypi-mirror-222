        ...
        for patch in patches:
            ...
            for oid, orev in patch.depends:
                ...
                assert crev is None or crev <= orev, (
                    f"The {patch} depends on {oid}@{orev}, but it's currently at {crev}"
                )
                if crev != orev:
                    other = manager[oid]
                    if other in patches:
            ...
            for oid, orev in patch.replaces:
                crev = context[oid]
                assert crev is None or crev <= orev, (
                    f"The {patch} replaces {oid}@{orev}, but it's currently at {crev}"
                )
                if crev != orev:
                    other = manager[oid]
                    if other in patches:
            ...
            for oid, orev in patch.preceeds:
                crev = context[oid]
                assert crev is None or crev >= orev, (
                    f"The {patch} preceeds {oid}@{orev}, but it's currently at {crev}"
                )
                if crev != orev:
                    other = manager[oid]
                    if other in patches:
            ...
            for oid, orev in patch.brings:
                ...
                crev = context[oid]
                assert crev is None or crev <= orev, (
                    f"The {patch} brings {oid}@{orev}, but it's currently at {crev}"
                )
