...
from daisytuner.measure import random_arguments, measure
...
class TransferTuner:
    def __init__(self, hostname: str, arch: str) -> None:
    ...
    def tune(
        ...
        nns = self._query_nearest_neighbors(
            loop_nest, topK=topK, static_only=static_only
        )
        if not nns:
            return None
        ...
        for nn in nns:
            ...
            recom, applied_schedule = self.apply_schedule(
                loop_nest, schedule, in_place=False, static_only=static_only
            )
            ...
            runtime, process_time, _ = measure(
                recom, arguments=args, timeout=best_process_time * 1.5, measurements=1
            )
            if runtime == math.inf:
                continue
        ...
        if best_schedule is not None:
            _, applied_schedule = self.apply_schedule(
                loop_nest, best_schedule, in_place=True, static_only=static_only
            )
        else:
            applied_schedule = []
    ...
    def _query_nearest_neighbors(
        self, loop_nest: LoopNest, topK: int, static_only: bool
    ) -> Dict:
        if static_only:
            runner = "__static"
            emb, _ = self._model.predict(loop_nest=loop_nest, static_only=static_only)
        else:
            emb, _, _, _, runner = self._model.predict(
                loop_nest=loop_nest, static_only=static_only
            )
        ...
        for trans in schedule:
            ...
            if static_only:
                _, node_embeddings = self._model.predict(
                    loop_nest_, static_only=static_only
                )
            else:
