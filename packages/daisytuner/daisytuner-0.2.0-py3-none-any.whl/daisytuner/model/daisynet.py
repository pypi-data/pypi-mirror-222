...
class DaisyNet(pl.LightningModule):
    ...
    def predict(
        self, loop_nest: LoopNest, static_only: bool = False
    ) -> Tuple[np.ndarray]:
        encoding = Encoding(
            loop_nest=loop_nest, hostname=self._hostname, arch=self._arch
        )
        data, mapping = encoding.encode(static_only=static_only)
        data.to(self.device)
    ...
    @classmethod
    def create(cls, hostname: str, arch: str) -> DaisyNet:
