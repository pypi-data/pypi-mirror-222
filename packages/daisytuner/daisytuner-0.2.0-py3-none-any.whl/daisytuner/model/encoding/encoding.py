...
class Encoding:
    ...
    def __init__(self, loop_nest: LoopNest, hostname: str, arch: str) -> None:
        ...
        self._architecture_encoding = ArchitectureEncoding(self._hostname)
        self._profiling_encoding = ProfilingEncoding.create(
            loop_nest, self._hostname, self._arch
        )
    ...
    def encode(self, static_only: bool = False):
        # Static Encoding
        ...
        if not static_only:
            data.arch = self._architecture_encoding.encode()
            data.counters = self._profiling_encoding.encode()
