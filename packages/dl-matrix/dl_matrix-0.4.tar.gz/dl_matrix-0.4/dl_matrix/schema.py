from typing import Type, List, Optional, Dict, Any, Union, Set
import uuid
from llama_index.schema import TextNode, NodeRelationship as DocumentRelationship
...
class ChainDocument(TextNode):
    ...
    create_time: Optional[float] = Field(
        default=None,
        description="The time when this document was created.",
    )
...
class ChainDocumentStore(BaseModel):
    ...
    def add_documents(
        ...
        """
        for doc in docs:
            if doc.id_ is None and generate_id:
                doc.id_ = self.get_new_id()
            self.docs[doc.id_] = doc
