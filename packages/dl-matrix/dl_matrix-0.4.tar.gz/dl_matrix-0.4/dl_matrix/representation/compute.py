...
from dl_matrix.structure import ChainTreeIndex, ChainMap
from dl_matrix.coordinate import Coordinate
from dl_matrix.representation.animate import animate_conversation_tree
from dl_matrix.schema import DocumentRelationship, ChainDocument
...
class CoordinateRepresentation(Representation):
    ...
    def _create_coordinates_graph(
        ...
        try:
            ...
            while stack:
                ...
                relationships[message_id][DocumentRelationship.CHILD] = children_ids

                sibling_coords = [
                    tetra_dict[sibling_id]
                    for sibling_id in children_ids
                    if sibling_id in tetra_dict
                ]

                # We first sort the children by time based on the message creation time
                ...
                for i, child_id in enumerate(
                    ...
                    try:
                        flattened_child_coordinate = self._assign_coordinates(
                            child_id, i, children_ids, sibling_coords, depth
                        )
                    ...
                    stack.append(
                        (
                            child_id,
                            flattened_child_coordinate,
                            depth + 1,
                        )
                    )
            print("tetra_dict", tetra_dict)
    ...
    def _procces_coordnates(
        ...
        try:
            ...
            tetra_dict, relationships = self._create_coordinates_graph(
                use_graph=use_graph
            )
            if not tetra_dict:
                raise ValueError("Failed to create coordinates.")
            if animate:
                animate_conversation_tree(
                    coordinates=list(tetra_dict.values()),
                    color_scalar=[0] * len(tetra_dict),
                )
        ...
        try:
            ...
            if local_embedding:
                try:
                    ...
                    message_embeddings = self.handler.semantic_similarity_model.generate_message_embeddings(
                    ...
                    logging.error(f"Error creating local embeddings: {e}")
                    return None
            ...
            logging.error(f"Error creating representation: {e}")
            return None
    ...
    def add_node_to_graph(
        ...
        Args:
            ...
            safe_tetra_dict: The dictionary containing the tetrahedron coordinates.
        ...
        if (
            ...
            try:
                graph.add_node(
                    message_id,
                    message=self.message_dict[message_id].message,
                )
            except Exception as e:
                logging.warning(f"Failed to add node for message {message_id}: {e}")
                failed_nodes.append(message_id)
        else:
            logging.warning(f"Invalid conditions for adding node {message_id}")
    ...
    def create_chain_document(
        ...
        """
        sub_graph = self.convert_subgraph_to_json(subgraph)
    ...
    def convert_subgraph_to_json(self, subgraph: nx.DiGraph) -> Dict:
        """
        Convert a networkx DiGraph into a json compatible format.

        Args:
            subgraph: A networkx DiGraph.

        Returns:
            The subgraph in json compatible format.
        """
        return json_graph.node_link_data(G=subgraph)
    ...
    def get_chain_document_parameters(
        ...
        return {
            ...
            "children": relationship.get(DocumentRelationship.CHILD, []),
            "author": author_role,
            "coordinate": coordinate,
            "sub_graph": sub_graph,
            "umap_embeddings": embedding_data[0],
