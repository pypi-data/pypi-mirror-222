...
class MetadataDetails(BaseModel):
    """
    Represents custom metadata for a message.
    Attributes:
        custom_key (str): The key for the custom metadata.
        custom_value (Any): The value for the custom metadata.
        metadata_template (str): Template for how metadata is formatted, with {key} and {value} placeholders.
        metadata_seperator (str): Seperator between metadata fields when converting to string.
        text_template (str): Template for how text is formatted, with {content} and {metadata_str} placeholders.

    """

    custom_key: Optional[str] = Field(
        default=DEFAULT_CUSTOM_METADATA_KEY,
        description="The key for the custom metadata.",
    )

    custom_value: Optional[Any] = Field(
        default=DEFAULT_CUSTOM_METADATA_KEY,
        description="The value for the custom metadata.",
    )
...
class Metadata(BaseModel):
    ...
    finish_details: Optional[MetadataFinishDetails] = Field(
        None, description="Details about the model's finish process."
    )
    metadata_details: Optional[Dict[str, MetadataDetails]] = Field(
        None, description="Custom metadata associated with the message."
    )
