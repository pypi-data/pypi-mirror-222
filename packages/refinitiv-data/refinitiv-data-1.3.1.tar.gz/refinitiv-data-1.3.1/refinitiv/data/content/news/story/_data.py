...
def story_rdp_from_dict(datum: dict) -> Story:
...
def story_udf_from_dict(datum: dict) -> StoryUDF:
    parser = HeadlineHTMLParser()
    story = datum["story"]
    for v in story.values():
        parser.feed(v)

    text = parser.data.get("text", [])
    text = "\n".join(text)
    html = parser.data.get("html", datum.get("story", {}).get("storyHtml", []))
    content = NewsStoryContent(html, text)
