...
def sync_regscale_and_jira(
    ...
    regscale_issues, regscale_attachments = fetch_regscale_issues(
        app=app,
        parent_id=parent_id,
...
def create_and_update_regscale_issues(args: Tuple, thread: int) -> None:
    ...
    for i in range(len(threads)):
        ...
        if jira_issue.fields.status.name.lower() == "done" and regscale_issue:
            ...
            if new_issue := Issue.insert_issue(
                app=app,
                issue=issue,
            ):
                if new_issue.id:
                ...
                new_regscale_issues.append(new_issue)
...
def fetch_regscale_issues(
    ...
    :return: Tuple[list[Issue], Optional[list[File]]]
    """
    attachments: Optional[dict] = None
