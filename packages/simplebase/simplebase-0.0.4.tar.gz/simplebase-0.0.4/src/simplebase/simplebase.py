...
def addFile(path):
    ...
    outfile=path
    with open(outfile, 'rb') as upd:
        blob.upload_from_file(upd)
...
def addFileAs(path, asto):
    bucket = storage.bucket()
    blob = bucket.blob(asto)
    outfile=path
    with open(outfile, 'rb') as upd:
        blob.upload_from_file(upd)

def publishFileAs(path, asto):
    bucket = storage.bucket()
    blob = bucket.blob(asto)
    outfile=path
    with open(outfile, 'rb') as upd:
...
def downloadFile(path, asto):
    bucket = storage.bucket()
    blob = bucket.blob(path)
    blob.download_to_filename(asto)
