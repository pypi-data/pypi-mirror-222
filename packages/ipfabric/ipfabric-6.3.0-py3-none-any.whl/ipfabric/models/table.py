...
DEVICE_EXPORT_FORMAT = Literal["json", "csv", "object"]
    ...
    def fetch(
        ...
        Args:
            ...
            csv_tz: str: Default None, set a timezone to return human-readable dates when using CSV;
                         see `ipfabric.tools.shared.TIMEZONES`
        Returns:
            Union[List[dict], Devices, bytes]: Default List of dicts 'json', Devices object if 'object', bytes if 'csv'
        """
        devices = super(DeviceTable, self).fetch(
            export="csv" if export == "csv" else "json",
            columns=columns,
            filters=filters,
            attr_filters=attr_filters,
            snapshot_id=snapshot_id,
