...
class InputOutputRunner:
    ...
    def __call__(self, inputs: Union[Type, List[Type]]) -> str:
        ...
        if isinstance(inputs, list):
            assert isinstance(inputs[0], self.input_type), "Input must be of type %s" % self.input_type
            # Just printing here if batched or not
            print("Running batch job on %d number of inputs" % len(inputs))
        else:
            # Singleton
            assert isinstance(inputs, self.input_type), "Input must be of type %s" % self.input_type
    ...
    def load_data(self, data, verbose: bool = False):
        ...
        assert isinstance(data, list) and isinstance(data[0], dict) and "input" in data[0] and "output" in data[0] and isinstance(data[0]["input"], dict) and isinstance(data[0]["output"], dict), f"Data must be [{'input': input_dict, 'output': output_dict}, {'input': input_dict, 'output': output_dict}, ...], that is a list of dicts (with inner input and output dicts), where inner dicts have keys matching input keys: {self.input_type_keys}; and, output keys: {self.output_type_keys} respectively."

        # Check if data is has the right input and output type keys
        input_datum = data[0]["input"]
        input_keys = input_datum.keys()
        assert input_keys == self.input_type_keys, f"Keys have to match {self.input_type}. But got {input_keys} instead."

        output_datum = data[0]["output"]
        output_keys = output_datum.keys()
        assert output_keys == self.output_type_keys, f"Keys have to match {self.output_type}. But got {output_keys} instead."
    ...
    def load_data_from_paired_lists(self, data, verbose: bool = False):
        ...
        assert isinstance(data, list) and isinstance(data[0], list) and isinstance(data[0][0], dict) and isinstance(data[0][1], dict), f"Data must be [[input_dict, output_dict], [input_dict, output_dict], ...], that is a list of lists (inner list is an input-output pair), where input and output are dicts with keys matching input keys: {self.input_type_keys}; and, output keys: {self.output_type_keys} respectively."
    ...
    def load_data_from_dataframe(self, df: pd.DataFrame, verbose: bool = False, input_prefix: str = "input-", output_prefix: str = "output-"):
        ...
        assert len(input_keys) > 0, f"Input keys must start with {input_prefix}. But got {input_keys} instead."
        assert len(output_keys) > 0, f"Output keys must start with {output_prefix}. But got {output_keys} instead."
        ...
        assert set(input_df.columns) == set(self.input_type_keys), f"Keys have to match " + str(self.input_type) + ". But got " + str(input_df.columns) + " instead."
        ...
        # Check if matching input and output keys
        assert set(output_df.columns) == set(self.output_type_keys), f"Keys have to match " + str(self.output_type) + ". But got " + str(output_df.columns) + " instead."
