...
integerizer = RetVecIntegerizer(replacement_int=65533)
...
def read_tfrecord(tfrecord: Tensor, binarizer: RetVecBinarizer) -> Dict[str, Tensor]:
    ...
    aug_token0_utf8 = tf.clip_by_value(integerizer.integerize(rec["aug_token0"]), 0, 65535)
    aug_token1_utf8 = tf.clip_by_value(integerizer.integerize(rec["aug_token1"]), 0, 65535)
    ...
    record["aug_decoder_utf8"] = tf.stack([aug_token0_utf8, aug_token1_utf8])
...
def get_process_tfrecord_fn(outputs: Set[str]) -> Callable:
    ...
    def process_tfrecord(e):
        ...
        y = {
            ...
            "aug_decoder_utf8": e["aug_decoder_utf8"]
        }
...
def get_dataset_samplers(
    bucket: str, train_path: str, test_path: str, config: Dict, decoder_only: bool = False
    ...
    if decoder_only:
        _, _, outputs = get_outputs_info_decoder()
    else:
...
def get_outputs_info_decoder() -> Tuple[List[Any], List[List[str]], Set[str]]:
    # loss = ["sparse_categorical_crossentropy"]
    # metrics = ["acc"]
    # outputs = ["aug_decoder_utf8"]
    
    loss = ["mse"]
    metrics = ["binary_crossentropy", "accuracy", "binary_accuracy", "mae"]
    outputs = ["aug_decoder"]
...
def get_outputs_info(config: Dict) -> Tuple[List[Any], List[List[str]], Set[str]]:
    ...
    if config_outputs.get("similarity_dim"):
    ...
    if config_outputs.get("original_decoder_size"):
        loss.append("binary_crossentropy")
        metrics.append(["mse"])
        outputs.add("ori_decoder")

    if config_outputs.get("aug_decoder_size"):
        loss.append("binary_crossentropy")
        metrics.append(["mse"])
        outputs.add("aug_decoder")

    if config_outputs.get("aug_decoder_utf8_size"):
        loss.append("sparse_categorical_crossentropy")
        metrics.append(["acc"])
        outputs.add("aug_decoder_utf8")

    if config_outputs.get("aug_vector_dim"):
        loss.append("binary_crossentropy")
        metrics.append(["acc", "binary_accuracy"])
        outputs.add("aug_vector")

    if config_outputs.get("aug_matrix_dim"):
        loss.append("binary_crossentropy")
        metrics.append(["acc", "binary_accuracy"])
