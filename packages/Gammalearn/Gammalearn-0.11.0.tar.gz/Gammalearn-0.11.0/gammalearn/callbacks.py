...
from torchmetrics import JaccardIndex, Accuracy, ConfusionMatrix
import numpy as np
import pandas as pd
import tables
...
class WriteAutoEncoder(Callback):
    """
    Callback to produce testing result data files
    Parameters
    ----------
    ...
    def on_test_end(self, trainer, pl_module):
        ...
        dataset_name = trainer.test_dataloaders[0].dataset.dataset_name
        output_path = os.path.join(output_dir, dataset_name) + '.csv'
...
class WriteAccuracy(Callback):
    ...
    def on_test_end(self, trainer, pl_module):
        ...
        output_path = os.path.join(output_dir, 'accuracy.csv')
...
class WriteAccuracyDomain(Callback):
    """
    Callback to produce testing result data files
    Parameters
    ----------
    ...
    def on_test_end(self, trainer, pl_module):
        ...
        output_dir = os.path.join(pl_module.experiment.main_directory,
                                  pl_module.experiment.experiment_name,
                                  'accuracy_domain_results')
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)

        output_path = os.path.join(output_dir, 'accuracy_domain.csv')
...
class WriteConfusionMatrix(Callback):
    """
    Callback to produce testing result data files
    Parameters
    ----------
