    ...
    def __init__(self, settings):
        ...
        if self.train:
            ...
            if hasattr(settings, 'train_files_max_number'):
                self._is_of_type('train_files_max_number', (int, dict))
...
class LitGLearnModule(LightningModule):
    def __init__(self, experiment):
        ...
        freeze_backbone = self.experiment.net_parameters_dic['parameters'].get('freeze_backbone', False)
        if freeze_backbone:
            assert hasattr(self.net, 'freeze_backbone'), 'freeze_backbone method not implemented for current model'
            self.net.freeze_backbone()
            self.console_logger.info('freezing backbone')
