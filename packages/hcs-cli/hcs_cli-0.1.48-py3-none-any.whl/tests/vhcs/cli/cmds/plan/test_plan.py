...
class TestPlan(CliTest):
    ...
    def test11_basic_dep(self):
        ...
        bp = _get_blueprint(dpid)
        self.verify_deploy(bp)
        self.verify_execution_log(dpid, 'create', "r2 must be deployed after r1", precise_order=['start/r1', 'success/r1', 'start/r2', 'success/r2'])
        self.verify_destroy(bp)
        self.verify_execution_log(dpid, 'delete', "r2 must be destroyed before r1", precise_order=['start/r2', 'success/r2', 'start/r1', 'success/r1'])
    
    def test12_basic_dep_double(self):
        dpid = 'd12'
        bp = _get_blueprint(dpid)
        self.verify_deploy(bp)
        self.verify_execution_log(dpid, 'create', "r2 must be deployed after r1a", partial_order=['success/r1a', 'start/r2'])
        self.verify_execution_log(dpid, 'create', "r2 must be deployed after r1b", partial_order=['success/r1b', 'start/r2'])
        self.verify_destroy(bp)
        self.verify_execution_log(dpid, 'delete', "r2 must be destroyed before r1a", partial_order=['success/r2', 'start/r1a'])
        self.verify_execution_log(dpid, 'delete', "r2 must be destroyed before r1a", partial_order=['success/r2', 'start/r1b'])

    def test20_basic_parallel(self):
        dpid = 'd20'
        bp = _get_blueprint(dpid)
        self.verify_deploy(bp)
        self.verify_execution_log(dpid, 'create', "success of r2 must before success of r1", partial_order=['success/r2', 'success/r1'])
        self.verify_destroy(bp)
        self.verify_execution_log(dpid, 'delete', "both r1 and r2 must be destroyed", any_order=['success/r2', 'success/r1'])
    
    def test21_basic_parallel2(self):
        dpid = 'd21'
        bp = _get_blueprint(dpid)
        self.verify_deploy(bp)
        self.verify_execution_log(dpid, 'create', "r2b and r2c runs in parallel as r1", partial_order=['start/r1', 'start/r2b', 'success/r2b', 'start/r2c', 'success/r2c', 'success/r1'])
        self.verify_destroy(bp)
    ...
    def test30_basic_error_sequential(self):
        ...
        self.verify_execution_log(dpid, 'create', "r2 must not be deployed, due to failure in r1", precise_order=['start/r1', 'error/r1'])
        self.verify_destroy(bp)
        self.verify_execution_log(dpid, 'delete', "r2 must not be destroyed, since it's not deployed", precise_order=['skip/r2', 'start/r1', 'success/r1'])

    def test31_basic_error_parallel(self):
        dpid = 'd31'
        bp = _get_blueprint(dpid)
        self.verify_deploy(bp, expected_return_code=1)
        self.verify_execution_log(dpid, 'create', "success of r2 must before error of r1", partial_order=['success/r2', 'error/r1'])
        self.verify_destroy(bp)
        self.verify_execution_log(dpid, 'delete', "both r1 and r2 must be cleaned up", any_order=['success/r2', 'success/r1'])
    ...
    def test60_basic_statement_for(self):
        ...
        bp = _get_blueprint(dpid)
        self.verify_deploy(bp)
        self.verify_execution_log(dpid, 'create', "r1 must be deployed twice with an additional one for the group", partial_order=['success/r1#0', 'success/r1#1', 'success/r1'])
        self.verify_destroy(bp)
        self.verify_execution_log(dpid, 'delete', "r1 must be destroyed twice with an additional one for the group", any_order=['success/r1#0', 'success/r1#1', 'success/r1'])

    def test70_list_map_expression(self):
        dpid = 'd70'
        bp = _get_blueprint(dpid)
        self.verify_deploy(bp)
        self.verify_execution_log(dpid, 'create', "three r1 instances must be created before r2 start", partial_order=['success/r1#0', 'success/r1#1', 'success/r1', 'start/r2', 'success/r2'])
        self.verify_output(dpid, "output.r2.agg", ['a@t.com', 'b@t.com'])
        self.verify_destroy(bp)
        self.verify_execution_log(dpid, 'delete', "r1 group must be deleted after r2", partial_order=['start/r2', 'success/r2', 'start/r1', 'start/r1#0'])

    def test80_basic_condition(self):
        dpid = 'd80'
        bp = _get_blueprint(dpid)
        self.verify_deploy(bp)
        self.verify_execution_log(dpid, 'create', "r1 and r11 are deployed. r2 and r21 are not.", partial_order=['success/r1', 'success/r11'], any_order=['skip/r2', 'skip/r21'])
        self.verify_destroy(bp)
        self.verify_execution_log(dpid, 'delete', "r1 and r11 are destroyed. r2 and r22 are not.", partial_order=['success/r11', 'success/r1'], any_order=['skip/r2', 'skip/r21'])
