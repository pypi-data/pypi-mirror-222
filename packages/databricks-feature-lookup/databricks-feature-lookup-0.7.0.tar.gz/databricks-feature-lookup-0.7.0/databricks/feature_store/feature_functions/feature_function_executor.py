...
class FeatureFunctionExecutor:
    ...
    def execute_feature_functions(self, df: pd.DataFrame):
        """
        ...
        def _execute_on_demand_computation_on_row(
            ...
            return [
                ...
                for feat in self.materializable_features.values()
            ]

        uncasted_df = df.apply(
            _execute_on_demand_computation_on_row, axis=1, result_type="expand"
        )
        uncasted_df.columns = list(self.materializable_features.keys())
    ...
    def _convert_output_types(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        spark_schema = [
            (output_name, feat.return_data_type)
            for output_name, feat in self.materializable_features.items()
        ]
