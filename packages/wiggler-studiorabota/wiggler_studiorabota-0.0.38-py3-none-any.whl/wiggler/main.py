...
@app.get("/take_picture", tags=[TAG_IMAGE])
def take_picture():
    now = datetime.now()
...
@app.get("/schedule_picture/{minutes}", tags=[TAG_IMAGE])
...
@app.get("/timelapse/schedule", tags=[TAG_VIDEO])
...
@app.delete("/zip/{date}", tags=[TAG_ZIP])
...
@app.get("/zip/stream/{date}", tags=[TAG_ZIP])
...
@app.get("/zip/write/yesterday", tags=[TAG_ZIP])
async def zip():
    yesterday = (datetime.now() - timedelta(1)).strftime('%Y-%m-%d')
    os.system(
        f'zip {ZIP_FOLDER}/{yesterday} {IMG_FOLDER}/{yesterday}*.jpg')
    return {
        "name": yesterday,
...
@app.get("/zip/schedule", tags=[TAG_ZIP])
def schedule_timelapse():
    cron = CronTab(user=os.getlogin())
