...
class BaseAddOn:
            ...
            with open("config.yaml") as config:
                ...
                if "files" in schema:
                    for field in schema["files"]:
                        resp = self.client.post("addon_files/")
                        with open(args["data"][field]) as file:
                            requests.put(resp["presigned_url"], data=file)
                        args["data"][field] = resp["uuid"]
        except FileNotFoundError:
            pass
        except fastjsonschema.JsonSchemaException as exc:
            print(exc.message)
    ...
    def get_file(self, field):
        """Get a file uploaded for addon usage"""
        file_uuid = self.data[field]
        resp = self.client.get(f"addon_files/{file_uuid}/")
