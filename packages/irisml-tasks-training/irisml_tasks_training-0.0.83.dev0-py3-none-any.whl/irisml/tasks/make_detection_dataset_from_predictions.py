...
class Task(irisml.core.TaskBase):
    ...
    @dataclasses.dataclass
    class Config:
        box_format: typing.Literal['xyxy', 'xywh'] = 'xyxy'
    ...
    def execute(self, inputs):
        predictions = [torch.tensor(p) for p in inputs.predictions]
        ...
        if self.config.box_format == 'xywh':
            predictions = [torch.cat((t[:, :-2], (t[:, -2] + t[:, -4]).unsqueeze(-1), (t[:, -3] + t[:, -1]).unsqueeze(-1)), dim=1) for t in predictions]
