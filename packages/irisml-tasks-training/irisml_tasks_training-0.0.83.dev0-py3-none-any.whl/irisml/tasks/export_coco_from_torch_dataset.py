...
class Task(irisml.core.TaskBase):
    ...
    def execute(self, inputs):
        ...
        dirpath = pathlib.Path(self.config.dirpath)
        if (dirpath / f'{self.config.json_file_name}.json').exists():
            raise RuntimeError(f'{self.config.json_file_name}.json already exists in {dirpath}!')

        image_dir = dirpath / self.config.image_directory_name
        ...
        (dirpath / f"{self.config.json_file_name}.json").write_text(json.dumps(coco, indent=2))
        return self.Outputs()
