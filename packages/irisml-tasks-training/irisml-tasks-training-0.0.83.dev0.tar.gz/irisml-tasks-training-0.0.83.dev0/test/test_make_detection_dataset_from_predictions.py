...
class TestMakeClassificationDatasetFromObjectDetection(unittest.TestCase):
    ...
    def test_xywh_to_xyxy(self):
        dataset = self.build_dataset()
        predictions_xywh = [torch.tensor([[0, 0.1, 0.1, 0.4, 0.5], [1, 0.1, 0.1, 0.2, 0.2]]), torch.empty(0, 5), torch.tensor([[2, 0, 0, 1, 1]])]
        outputs = Task(Task.Config(box_format='xywh')).execute(Task.Inputs(dataset, predictions_xywh))
        self.assertTrue(torch.equal(outputs.dataset[0][1], torch.tensor([[0, 0.1, 0.1, 0.5, 0.6], [1, 0.1, 0.1, 0.3, 0.3]])))
        self.assertTrue(torch.equal(outputs.dataset[1][1], torch.empty(0, 5)))
        self.assertTrue(torch.equal(outputs.dataset[2][1], torch.tensor([[2, 0, 0, 1, 1]])))
    ...
    def test_predictions_with_score(self):
        ...
        outputs = Task(Task.Config(box_format='xyxy', score_threshold=0.5)).execute(Task.Inputs(dataset, predictions_with_scores))
    ...
    def test_predictions_with_score_xywh(self):
        dataset = self.build_dataset()
        predictions_with_scores_xywh = [torch.tensor([[0, 0.8, 0.1, 0.1, 0.4, 0.5], [1, 0.3, 0.1, 0.1, 0.2, 0.2]]), torch.empty(0, 6), torch.tensor([[2, 0.2, 0, 0, 1, 1]])]
        outputs = Task(Task.Config(box_format='xywh', score_threshold=0.5)).execute(Task.Inputs(dataset, predictions_with_scores_xywh))
