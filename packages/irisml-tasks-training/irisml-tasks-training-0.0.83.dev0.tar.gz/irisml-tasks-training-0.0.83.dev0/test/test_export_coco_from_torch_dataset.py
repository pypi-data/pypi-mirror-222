...
class TestExportCOCOJsonFromDataset(unittest.TestCase):
    def test_classification(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_dir = pathlib.Path(temp_dir)
            json_file_name = 'foo'
            Task(Task.Config(task_type='classification_multiclass', json_file_name=json_file_name, dirpath=temp_dir)).execute(Task.Inputs(dataset=multilabel_dataset, class_names=["dog", "cat"]))
