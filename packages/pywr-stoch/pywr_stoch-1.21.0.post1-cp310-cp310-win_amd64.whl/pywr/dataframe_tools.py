...
def align_and_resample_dataframe(df, target_index, resample_func="mean"):
    """Align a DataFrame to the provided index.
    ...
    # NOTE: Pandas handles resampling using to_timestamp() which does not support
    # large dates yet. Raise an error if the frequency is not the same
    name = df.name if hasattr(df, "name") else "Anonymous"
    if model_freq != df_freq:
        raise ValueError(
            f"The DataFrame ({name}) must have the same model "
            f"frequency ({model_freq})"
        )

    # check DataFrame start and end dates (these were checked during resampling)
    if df.index[0] > start:
        raise ValueError(
            f"The DataFrame start date ({df.index[0]}) be smaller than the model "
            f"start date ({start})"
        )
    if df.index[-1] < end:
        raise ValueError(
            f"The DataFrame end date ({df.index[-1]}) be smaller than the model "
            f"end date ({end})"
        )

    df = df[start:end]
    if not df.index.equals(target_index):
        raise ValueError("The DataFrame index does not have the same model index")
