...
class Event(object):
    ...
    @property
    def duration(self):
        return (self.end.period - self.start.period).delta.total_seconds() / 86400
...
class EventRecorder(Recorder):
    ...
    def to_dataframe(self):
        ...
        for i, evt in enumerate(self.events):
            ...
            start[i] = evt.start.period
            end[i] = evt.end.period
...
class EventDurationRecorder(Recorder):
    ...
    def finish(self):
        ...
        df["duration"] = df.apply(
            lambda row: int((row["end"] - row["start"]).delta.total_seconds() / 86400),
            axis=1,
        )
        # Drop other columns
