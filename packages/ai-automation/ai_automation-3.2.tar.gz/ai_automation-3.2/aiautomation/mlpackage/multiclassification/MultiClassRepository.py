...
class MultiClassifyRepository(Repository):
    ...
    def run_all_visualize_models(self, x_train, x_val, y_train, y_val):
        ...
        super().run_all_model_search(hyper_parameter_tuning, self
                                     .create_model_visualize_entity_array(models, saved_model_array),
    ...
    def create_model_visualize_entity_array(models, saved_model_array):
        ...
        for model_path in saved_model_array:
            ...
            search_type = super().check_and_get_search_type(model)
            file_name = model_path.split(Variable.locationSeparator)[-1].removesuffix(Variable.pickleExtension)
            saved_entity_array.append(ModelVisualizeEntity(model, models.check_and_get_grid(model_type, search_type),
                                                           model_type, search_type, file_name))
